...
void TemplateTable::_new() {
  ...
  // 调用load_unsigned_short()函数加载new指令后的操作数并保存到%rdx中
  ...
  // 接下来会在TemplateTable::_new()函数中调用get_cpool_and_tags()函数获取常量池首地址并放入%rcx中，
  // 获取常量池中元素类型数组_tags的首地址并放入%rax中
  ...
  // 此时%rcx中存放的是InstanceKlass实例的内存地址，利用偏移获取创建的Java对象所需的内存并保存到%rdx中
  __ movl(rdx,
          Address(rsi,
