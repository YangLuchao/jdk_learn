...
// 调用InterpreterMacroAssembler::get_unsigned_2_byte_index_at_bcp()函数加载new指令后的操作数
void InterpreterMacroAssembler::get_unsigned_2_byte_index_at_bcp(
  Register reg,
  int bcp_offset) {
  assert(bcp_offset >= 0, "bcp is still pointing to start of bytecode");
  // 调用load_unsigned_short()函数加载new指令后的操作数并保存到%rdx中。对于实例9-1(对象的创建 文件)来说，这个值就是常量池的下标索引1。
  /*
  以下是调用函数生成的机器码对应的汇编代码：
...
在调用函数生成汇编代码（就是生成的机器码对应的汇编代码，因为汇编代码可读性强，所以后面都会这样叙述）时，
调用者会根据调用约定保证%r13中的值是bcp（byte code pointer），也就是保存着new字节码指令的地址。
  */
