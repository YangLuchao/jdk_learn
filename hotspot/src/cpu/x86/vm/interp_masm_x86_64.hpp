...
0x00007fffe1022b26: mov    0x10(%rsi),%rax  
  */
  // 由于Method实例中保存着ConstMethod指针，而ConstMethod中保存着指向ConstantPool的指针，所以最终的相关信息都是可以顺利找到的。
  void get_method(Register reg) {
    // %rbp指向栈底，通过固定偏移frame::interpreter_frame_method_offset * wordSize
    // 后就可以找到当前执行的new字节码指令所属的Method实例（这是HotSpot VM在实现Java栈桢时的本地约定）。
  ...
  // 在TemplateTable::_new()函数中调用get_cpool_and_tags()函数获取常量池首地址并放入tags中，获取常量池中元素类型数组_tags的首地址并放入tags中
