...
/// @param  
/// @return
// 调用TypeArrayKlass::create_klass()函数创建TypeArrayKlass实例 
...
// 创建typeArrayOop类实例，创建基本类型数组实例
// 参数length表示创建数组的大小，而do_zero表示是否需要在分配数组内存时将内存初始化为零值。
typeArrayOop TypeArrayKlass::allocate_common(int length, bool do_zero, TRAPS) {
  assert(log2_element_size() >= 0, "bad scale");
  if (length >= 0) {
    if (length <= max_length()) {
      // 首先调用typeArrayOopDesc::object_size()函数从_layout_helper中获取数组的大小，
      size_t size = typeArrayOopDesc::object_size(layout_helper(), length);
      KlassHandle h_k(THREAD, this);
      typeArrayOop t;
      CollectedHeap* ch = Universe::heap();
      // 然后调用array_allocate()或array_allocate_nozero()函数分配内存并初始化对象头，即为length、_mark和_metadata属性赋值。
      // 在TypeArrayKlass::allocate_common()函数中获取TypeArrayOopDesc实例需要分配的内存空间后，会调用CollectedHeap::array_allocate()或CollectedHeap::array_allocate_nozero()函数在堆上分配内存空间，然后初始化对象头信息。
      if (do_zero) {
