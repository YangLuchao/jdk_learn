...
// ArrayKlass的子类中有表示数组组件类型是Java基本类型的TypeArrayKlass，
// 以及表示组件类型是对象类型的ObjArrayKlass，
// 数组类和普通类不同，数组类没有对应的Class文件，因此数组类是虚拟机直接创建的
// HotSpot VM在初始化时就会创建Java中8个基本类型的一维数组实例TypeArrayKlass。
// 前面在介绍HotSpot VM启动时讲过initializeJVM()函数，这个函数会调用Universe::genesis()函数，
// 在Universe::genesis()函数中初始化基本类型的一维数组实例TypeArrayKlass
class TypeArrayKlass : public ArrayKlass {
 ...
 public:
  ...
  static TypeArrayKlass* create_klass(BasicType type, const char* name_str,
                               TRAPS);
  //v调用TypeArrayKlass::create_klass()函数创建TypeArrayKlass实例                        
  ...
  // 然后调用TypeArrayKlass类中的allocate()函数创建typeArrayOop实例，
