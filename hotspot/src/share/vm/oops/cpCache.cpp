...
// 初始化ConstantPoolCache
// 从inverse_index_map中取出原常量池索引后将其存储到_indices属性中。
// 前面介绍过，_indices的低16位存储的是原常量池索引，传递的参数也不会超过16位所能表达的最大值。
// 对于_f1暂时初始化为NULL，_f2与_flags暂时初始化为0，在Java运行过程中会更新这些字段的值。
