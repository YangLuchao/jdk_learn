...
class SymbolHashMap;
// CPSlot表示常量池中的一个槽位，借助此工具类可以从槽中获取指向Symbol实例的指针，还可以获取指向Klass实例的指针。
// 类描述符最终会解析为一个指向Klass实例的指针，因此最终槽中存储的指向Symbol实例（表示类型描述符）的指针会更新为指向Klass实例的指针
...
// 常量池类
// ConstantPool实例表示常量池，也是类元信息的一部分，因此继承了类Metadata
class ConstantPool : public Metadata {
 ...
 private:
  // 每个常量池项的类型
  // _tags表示每个常量池项的类型，常量池中的总项数通过_length保存，
  // 因此_tags数组的长度也为_length，具体存储的内容就是每一项的tag值，这是虚拟机规范定义好的
 ...
 private:
  // base()是ConstantPool类中定义的函数，因此this指针指向当前ConstantPool实例在内存中的首地址，
  // 加上ConstantPool类本身需要占用的内存大小的值之后，指针指向了常量池的数据区。
  // 数据区通常就是length个指针宽度的数组，其中length为常量池数量。
  ...
  CPSlot slot_at(int which) const {
    ...
    // 调用了obj_at_addr_raw()函数获取ConstantPool中对应索引处存储的值，然后封装为CPSlot实例返回
  ...
  intptr_t* obj_at_addr_raw(int which) const {
    ...
    // 加上ConstantPool类本身需要占用的内存大小的值之后，指针指向了常量池的数据区。
    // 数据区通常就是length个指针宽度的数组，其中length为常量池数量。
    // 通过(intptr_t*)&base()[which]获取常量池索引which处保存的值，
 ...
 public:
  ...
  void klass_at_put(int which, Klass* k) {
    ...
    // _tags数组中存储的类型也应该更新为JVM_CONSTANT_Class
    release_tag_at_put(which, JVM_CONSTANT_Class);
  }

  // For temporary use while constructing constant pool
  // 常量池项的下标与数组的下标是相同的，也就是说，如果当前的JVM_CONSTANT_Class项存储在常量池中下标为1的位置，
  // 则也要将其存储到tags数组中下标为1的位置，同时要将名称索引name_index保存到ConstantPool数据区（Length个指针代表的区域）的对应位置上
  ...
  // field_at_put()函数存储class_index与name_and_type_index属性的值
  ...
  // Tag query
  // 查询到witch所表示类的tag
  // constantTag可以看作是一个操作tag的工具类，里面提供了许多判断函数，如提供了is_klass()函数判断tag是否为JVM_CONSTANT_Class常量池项等
  ...
  // 类索引和父类索引都指向常量池中的JVM_CONSTANT_Class常量池项，所以如果类还没有执行连接操作，获取的是指向Symbol实例的指针，如果已经连接，可以获取指向Klass实例的指针
  ...
  // 在解析当前类索引之前，其实会调用ClassFileParser::parse_constant_pool()函数解析常量池
  // 在解析常量池的过程中，会将CONSTANT_Class_info项最终解析为一个指向Symbol实例的指针，
  // 因为该项中有用的信息是一个name_index，这是一个常量池下标索引，在name_index索引处存储着CONSTANT_Utf8_info项，
  // 此项用一个字符串表示类的全限定名，而在HotSpot VM中，所有的字符串都用Symbol实例来表示，因此最终在解析常量池时，
  // 会在存储CONSTANT_Class_info项的下标索引处存储一个指向Symbol实例的指针
  /*
  #3 = Class         #17        // TestClass
  ...
  #17 = Utf8          TestClass
  在以上代码中，类索引为3，在常量池里找索引为3的类描述符，类描述符中的索引为17，再去找索引为17的字符串，即TestClass
  调用obj_at_addr_raw()函数找到的是一个指针，这个指针指向表示TestClass字符串的Symbol实例，
  也就是在解析常量池项时会将本来存储的索引值为17的位置替换为存储指向Symbol实例的指针。
  */
  ...
  // _tags数组中保存的tag值更新为JVM_CONSTANT_UnresolvedClass
  // 调用ConstantPool类中定义的symbol_at()函数从常量池which索引处获取表示签名字符串的Symbol实例
 ...
 public:
  ...
  // 由size()函数的计算方式可知，ConstantPool实例的内存布局其实就是ConstantPool本身占用的内存大小加上length个指针长度
  // ConstantPool实例布局图，看test/ylctest/4/4.2
