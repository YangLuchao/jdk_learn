...
class vtableEntry;
// klassVtable与klassItable类用来实现Java方法的多态，也可以称为动态绑定，
// 是指在应用执行期间通过判断接收对象的实际类型，然后调用对应的方法。
// C++为了实现多态，在对象中嵌入了虚函数表vtable，通过虚函数表来实现运行期的方法分派，
// Java也通过类似的虚函数表实现Java方法的动态分发。
// C++中的vtable只包含虚函数，非虚函数在编译期就已经解析出正确的方法调用了。
// Java的vtable除了虚方法之外还包含其他的非虚方法。
// vtable在内存中的布局如图所示：test/ylctest/6/6.3/vtable在Klass中的内存布局.png
...
class klassVtable : public ResourceObj {
  ...
  // vtable的长度，即vtableEntry的数量。因为一个vtableEntry实例只包含一个Method*，
  // 其大小等于字宽（一个指针的宽度），所以vtable的长度跟vtable以字宽为单位的内存大小相同。
 ...
 public:
  // 调用的构造函数中会初始化_tableOffset与_length属性。
  // _tableOffset表示相对Klass实例首地址的偏移量，length表示vtable中存储的vtableEntry的数量。
...
// vtable表示由一组变长（前面会有一个字段描述该表的长度）连续的vtableEntry元素构成的数组。
// 其中，每个vtableEntry封装了一个Method实例。
...
// 每个偏移表itableOffset保存的是类实现的一个接口Klass和该接口方法表所在的偏移位置；
...
// 方法表itableMethod保存的是实现的接口方法。
// tableMethodEntry也是对Method的一个封装
...
//    ...
//
// klassItable与klassVtable的作用类似，都是为了实现Java方法运行时的多态，但通过klassItable可以查找到某个接口对应的实现方法。
// itable是Java接口函数表，可以方便查找某个接口对应的实现方法。itable的结构比vtable复杂，除了记录方法地址之外还要记录该方法所属的接口类Klass
// itable结构如图：test/ylctest/6/6.5/itable的结构.png
// itable表由偏移表itableOffset和方法表itableMethod两个表组成，这两个表的长度是不固定的，即长度不一样。
// 调用接口方法时，HotSpot VM通过ConstantPoolCacheEntry的_f1成员拿到接口的Klass，在itable的偏移表中逐一匹配。如果匹配上则获取Klass的方法表的位置，
// 然后在方法表中通过ConstantPoolCacheEntry的_f2成员找到实现的方法Method。
class klassItable : public ResourceObj {
  /*
  增加itable而不用vtable解决所有方法分派问题，是因为一个类可以实现多个接口，
  而每个接口的函数编号是和其自身相关的，vtable无法解决多个对应接口的函数编号问题。
  而一个子类只能继承一个父亲，子类只要包含父类vtable，并且和父类的函数包含部分的编号是一致的，
 ...
 private:
  ...
  // 初始化itable
  // itable的itableOffset偏移表在类解析时完成初始化，而itable的方法表itableMethod需要等到方法连接时才会初始化
