...
ObjArrayKlass* ObjArrayKlass::allocate(ClassLoaderData* loader_data, int n, KlassHandle klass_handle, Symbol* name, TRAPS) {
  ...
  // 需要调用ArrayKlass::static_size()计算出ObjArrayKlass实例所需要的内存，
  int size = ArrayKlass::static_size(ObjArrayKlass::header_size());
  // 然后调用new重载运算符从元空间中分配指定大小的内存
  // 最后调用ObjArrayKlass的构造函数初始化相关属性
  return new (loader_data, size, THREAD) ObjArrayKlass(n, klass_handle, name);
}

// 创建组合类型为element_klass的n维数组
// allocate_objArray_klass()函数的参数element_klass有可能为TypeArrayKlass、InstanceKlass或ObjArrayKlass。
// 最终会调用ObjArrayklass::allocate()函数创建一个组合类型为element_klass、维度为n、名称为name的ObjArrayKlass
// 例如，TypeArrayKlass表示一维字节数组，n为2，name为[[B，最终会创建出对应的ObjArrayKlass实例。
// 最后还会调用ArrayKlass::complete_create_array_klass()函数完成_component_mirror等属性设置，
...
// ObjArrayKlass::array_klass函数创建n维类型数组
// 在InstanceKlass::array_klass_impl()函数中，如果创建好了一维类型的数组，依据这个一维类型数组就可以创建出n维类型数组，
// 无论调用array_klass()还是array_klass_or_null()函数，最终都会调用array_klass_impl()函数
  ...
  if (higher_dimension() == NULL) {
    ...
    JavaThread *jt = (JavaThread *)THREAD;
    {
      ...
      if (higher_dimension() == NULL) {
        ...
        // 多维数组会间接递归调用allocate_objArray_klass函数创建符合维度要求的数组类型。
        Klass* k =
