...
ConstantPool* ConstantPool::allocate(ClassLoaderData* loader_data, int length, TRAPS) {
  ...
  // 常量池项的数量
  // 调用ConstantPool::size()函数计算创建ConstantPool实例所需要分配的内存大小
...
// 调用ConstantPool类的构造函数初始化相关属性
// Java虚拟机规范中规定的常量池的具体格式：test/ylctest/4/4.2
// 在常量池解析过程中，通过常量池下标索引获取常量池项后会将tag属性的值放到ConstantPool实例的_tags数组中，数组的下标与常量池下标索引相对应。
// 剩下的信息只能存储到ConstantPool实例开辟的length个指针宽度的空间中，也可以看成是length长度的指针数组，其中的下标也与常量池下标索引对应。
// 例如，对于CONSTANT_Double_info来说，高位4个字节存储high_bytes，低位4个字节存储low_bytes。遇到CONSTANT_Utf8_info常量池项时，直接封装为Symbol实例，这样只要存储指向Symbol实例的指针即可。
ConstantPool::ConstantPool(Array<u1>* tags) {
  // 初始化tags、_length及_lock等属性，调用tags->length()获取的值就是常量池项的数量，因此_length属性保存的是常量池项的数量。
  ...
  for (int index = 0; index < length; index++) {
    // 在分析具体的常量池项时会更新为以下枚举类中定义的枚举常量
    // 源代码位置：openjdk/hotspot/src/share/vm/prims/jvm.h#enum
...
void ConstantPool::initialize_resolved_references(ClassLoaderData* loader_data,
                                                  // 参数reference_map为Rewriter类中的_resolved_references_map属性值。
                                                  // 在整数栈reference_map中存储着原常量池中的下标索引
...
// 在HotSpot VM运行过程中，如果需要获取Klass实例，而槽位上存储的是指向Symbol实例的指针时，就会调用ConstantPool::klass_at_impl()函数，
Klass* ConstantPool::klass_at_impl(constantPoolHandle this_oop, int which, TRAPS) {
 ...
 // 如果槽位上存储的是指向Klass实例的指针，则直接返回即可
  ...
  if (do_resolve) {
    ...
    // 调用如下函数获取Klass实例
    // SystemDictionary::resolve_or_fail()函数首先会从系统字典中查找对应的Klass实例，如果找不到，加载并解析类，最终会生成一个Klass实例并返回。
    ...
    if (TraceClassResolution && !k()->oop_is_array()) {
      ...
      if (do_resolve) {
        ...
        // 最终，实例4-2的常量池如图test/ylctest/4/4.2/ConstantPool实例布局4.png
...
Symbol* ConstantPool::klass_name_at(int which) const {
  ...
  // tag is not updated atomicly.
  // 拿到witch在常量池的slot实例
  // 这样在klass_name_at()函数中就可以根据地址adr的最后一位是0还是1来判断存储的到底是Klass还是Symbol实例
