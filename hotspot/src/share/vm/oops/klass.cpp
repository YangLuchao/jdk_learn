...
// 如果k存在于_secondary_supers中，需要调用search_secondary_supers()函数来判断父子关系
bool Klass::search_secondary_supers(Klass* k) const {
  ...
  // 查询的逻辑很简单，遍历_secondary_supers数组中的值并比较即可。
  for (int i = 0; i < cnt; i++) {
...
jint Klass::array_layout_helper(BasicType etype) {
  ...
  // 表示Java数组类型的_layout_helper属性由四部分组成
  // tag 如果数组元素的类型为对象类型，则值为0x80，否则值为0xC0，表示数组元素的类型为Java基本类型 枚举为：
  // openjdk/hotspot/src/share/vm/oops/klass.hpp
  // _lh_array_tag_type_value = ~0x00, _lh_array_tag_obj_value = ~0x01
  // hsize hsize表示数组头元素的字节数，调用arrayOopDesc::base_offset_in_bytes()及相关函数可以获取hsize的值
  // openjdk/hotspot/src/share/vm/oops/arrayOop.hpp
  // etype与esize etype表示数组元素的类型，esize表示数组元素的大小。
  // 最终会在Klass::array_layout_helper()函数中调用array_layout_helper()函数完成属性值的计算
...
/*
在设置当前类型的父类时通常都会调用initialize_supers()函数，
同时也会设置_primary_supers与_super_check_offset属性的值，
如果继承链过长，还有可能设置_secondary_supers、_secondary_super_cache属性的值。
这些属性中保存的信息可用来快速地进行类之间关系的判断，如父子关系的判断。
*/
void Klass::initialize_supers(Klass* k, TRAPS) {
  ...
  // k就是当前类的直接父类，如果有父类，那么super()一般为NULL，如果k为NULL，那么就是Object类
    ...
    // my_depth默认的值为8,将直接父类的继承类复制到_primary_supers中，因为直接父类和当前子类肯定有共同的继承链
  ...
  // 从代码中可以看到，父类继承链中多于8个时，多出来的父类被存储到_secondary_supers数组中。
  // 因为继承链一般不会多于8个，所以设置了默认值为8，避免过大的数组浪费太多的内存。
  // 看实例 2_1
...
// 调用Klass::append_to_sibling_list()函数设置_next_sibling与_subklass属性的值
