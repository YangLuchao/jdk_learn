...
InstanceKlass* InstanceKlass::allocate_instance_klass(
                                              ...
                                              bool is_anonymous,
                                              TRAPS) {
  // 在创建时首先需要分配内存，这涉及C++对new运算符重载的调用，通过重载new运算符的函数为对象分配内存空间，然后再调用类的构造函数初始化相关的属性                                            
  ...
  // Allocation
  InstanceKlass* ik;
  // 然后根据需要创建的类型rt创建不同的C++类实例
  // 当rt为REF_NONE时，普通的Java类通过InstanceKlass实例表示，java.lang.Class类通过InstanceMirrorKlass实例表示，java.lang.ClassLoader类通过Instance-ClassLoaderKlass实例表示。
  // 当rt不为REF_NONE时，会创建表示java.lang.Reference类的InstanceRefKlass实例。
  if (rt == REF_NONE) {
...
// 调用create_new_vtable_indices()函数根据默认方法的数量len初始化属性
...
// 在构造函数中会将除了header以外的内存初始化为0（在64位平台下，一个指针占8个字节，也就是一个字，因此以字为单位的初始化比以字节为单位的初始化速度更快），
// header就是InstanceKlass本身占用的内存经过8字节对齐后的值。
InstanceKlass::InstanceKlass(int vtable_len,
  ...
  // size.
  // 暂时将_layout_helper的值初始化为0，等类解析完成后会更新此值可以从更新后的值中获取创建实例
  //（当前InstanceKlass实例表示Java类，这里指oop实例，也就是Java对象）的内存空间
...
klassVtable* InstanceKlass::vtable() const {
  ...
  // vtable_length()函数用于获取Klass中_vtable_len属性的值，这个值在解析Class文件、创建Klass实例时已经计算好，这里只需要获取即
...
void InstanceKlass::initialize(TRAPS) {
  ...
  if (this->should_be_initialized()) {
    ...
    instanceKlassHandle this_oop(THREAD, this);
    // 调用InstanceKlass::initialize_impl()函数对类进行初始化。
...
// 调用rewrite_class()函数重写部分字节码。重写字节码大多是为了在解释执行字节码过程中提高程序运行的效率
...
void InstanceKlass::rewrite_class(TRAPS) {
  ...
  // 调用Rewriter::rewrite()函数进行字节码重写，重写完成后调用set_rewritten()函数记录重写完成的状态，保证任何类只会被重写一次
...
// 调用InstanceKlass::initialize_impl()函数对类进行初始化。
// 在对类进行初始化之前必须保证类已经完成连接。
// initialize_impl函数非常清晰地展示了类初始化的步骤。在类初始化的过程中会对类的状态进行判断。如果当前类正在被初始化，那么状态为being_initialized；
// 如果当前类已经完成初始化，则状态为fully_initialized；如果当前类初始化出错，则状态为initialization_error。
void InstanceKlass::initialize_impl(instanceKlassHandle this_oop, TRAPS) {
  ...
  // Step 1
  {
    ...
    // Step 3
    // 步骤3：当前类正在被当前线程初始化。例如，如果X类有静态变量指向new Y类实例，Y类中又有静态变量指向new X类实例，
  ...
  // 步骤8：执行类或接口的初始化方法<clinit>
  {
    ...
    // 调用类或接口的<clinit>方法
    // 在类初始化的过程中，最重要的就是调用类的<clinit>方法
  ...
  if (!HAS_PENDING_EXCEPTION) {
    ...
    // Step 10 and 11
    // 步骤10和11：如果初始化过程发生异常，则通过set_initialization_state_and_notify()方法设置类的状态为initialization_error并通知其他线程，然后抛出错误异常
    Handle e(THREAD, PENDING_EXCEPTION);
    CLEAR_PENDING_EXCEPTION;
...
// 创建instanceOop实例时会调用allocate_instance()函数
instanceOop InstanceKlass::allocate_instance(TRAPS) {
  bool has_finalizer_flag = has_finalizer(); // Query before possible GC
  // 调用了instanceKlass类中的size_helper()函数获取创建instanceOop实例所需要的内存空间
  int size = size_helper();  // Query before forming handle.

  KlassHandle h_k(THREAD, this);

  instanceOop i;
  // 调用了CollectedHeap::obj_allocate()函数分配size的内存,并将内存初始化为零值
...
// 创建Object一维数组的ObjArrayKlass实例，
// 首次创建ObjTypeKlass时，Instance-Klass::_array_klasses属性的值为NULL，
// 这样就会调用ObjArrayKlass::allocate_objArray_klass()函数创建一维对象数组并保存到InstanceKlass::_array_klasses属性中。
// 有了一维的对象类型数组后就可以接着调用array_klass_or_null()或array_klass()函数创建n维的对象类型数组了
Klass* InstanceKlass::array_klass_impl(instanceKlassHandle this_oop, bool or_null, int n, TRAPS) {
  if (this_oop->array_klasses() == NULL) {
    ...
    JavaThread *jt = (JavaThread *)THREAD;
    {
      ...
      // Java类型的Klass实例在HotSpot VM中唯一，所以当_array_klass为NULL时，表示首次以Klass为组件类型创建高一维的数组。
      // 创建成功后，将高一维的数组保存到_array_klass属性中，这样下次直接调用array_klasses()函数获取即可。
      if (this_oop->array_klasses() == NULL) {
        // 创建以当前InstanceKlass实例为基本类型的一维类型数组，创建成功后保存到_array_klasses,避免下次重新创建
...
// 最终通过调用JavaCalls::call()函数完成了Java方法的调用，这个函数非常重要，前面也多次使用过这个函数，
...
void InstanceKlass::do_local_static_fields_impl(instanceKlassHandle this_k,
  ...
  // 并对每个静态字段调用initialize_static_field()函数进行处理                             
...
// note that the local methods array can have up to one overpass, one static
// and one instance (private or not) with the same name/signature
// Find在本地方法数组中查找名称/签名，并在立交、静态和私有标志上进行筛选。这将返回找到的第一个。
// 注意，本地方法数组最多可以有一个立交、一个静态和一个具有相同名称/签名的实例(私有或非私有)
...
Method* InstanceKlass::find_method_impl(
    ...
    PrivateLookupMode private_mode) {
  // find_method_index()函数会对methods数组进行二分查找来搜索名称为name和签名为signature的方法。
...
// 调用find_method_index()函数使用二分查找算法在_methods属性中查找方法，
// 如果找到方法，则返回数组的下标位置，否则返回-1
...
Method* InstanceKlass::uncached_lookup_method(Symbol* name, Symbol* signature, OverpassLookupMode overpass_mode) const {
  ...
  while (klass != NULL) {
    ...
    // 如果调用find_method()函数无法从当前类中找到对应的方法，那么就通过while循环一直从继承链往上查找，如果找到就直接返回，否则返回NULL
...
Method* InstanceKlass::lookup_method_in_all_interfaces(Symbol* name,
  ...
  for (int i = 0; i < num_ifs; i++) {
    ...
    // 调用InstanceKlass::lookup_method()函数查找父类实现的接口中是否有名称为name、签名为signature的方法，
    // 如果查找到了public和非静态方法则直接返回，也就是说父类的vtable中已经存在名称为name、签名为signature的方法对应的vtableEntry，
    // 所以当前类中可重用这个vtableEntry，不需要新增vtableEntry
