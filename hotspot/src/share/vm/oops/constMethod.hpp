...
// 表示方法的不可变部分，入方法ID，方法字节码大小，方法名在常量池中的索引等
...
private:
  ...
  // 对于方法来说这是唯一ID，这个ID通常来说是methods数据的下标索引
...
public:
  ...
  // Sizing
  // 调用header_size()函数获取ConstMethod本身需要占用的内存空间
  ...
  void    set_code(address code) {
    if (code_size() > 0) {
      // 当字节码的大小不为0时，调用memcpy()函数将字节码内容存储在紧挨着Const-Method本身占用的内存空间之后
