...
// oopDesc是所有类名格式为xxxOopDesc类的基类，这些类的实例表示Java对象，
// 因此类名格式为xxxOopDesc的类中会声明一些保存Java对象信息的字段，这样就可以直接被C++获取
// Java对象内存布局主要分为header（头部）和fields（实例字段）。
// header由_mark和_metadata组成。
// _mark字段保存了Java对象的一些信息，如GC分代年龄、锁状态等；
// _metadata使用联合体（union）来声明，这样是为了在64位平台能对指针进行压缩。联合体中定义的_klass或_compressed_klass指针指向的是Klass实例，这个Klass实例保存了Java对象的实际类型，也就是Java对象所对应的Java类。
...
class oopDesc {
 ...
 private:
  // Java对象的header信息可以存储在oopDesc类中定义的_mark和_metadata属性中
  volatile markOop  _mark;
  union _metadata {
    Klass*      _klass;
    narrowKlass _compressed_klass;
  } _metadata;
  // 而Java对象的fields没有在oopDesc类中定义相应的属性来存储，因此只能申请一定的内存空间，然后按一定的布局规则进行存储。
  // 对象字段存放在紧跟着oopDesc实例本身占用的内存空间之后，在获取时只能通过偏移来取值。
 ...
 public:
  ...
  // 计算占用的字的大小，对于64位平台来说，一个字的大小为8字节，因此HeapWord-Size的值为8。
