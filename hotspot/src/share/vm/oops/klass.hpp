...
class Klass : public Metadata {
 ...
 protected:
  ...
  // ylc
  // 对类布局的综合描述符，如果不是InstanceKlass或 ArravKlass，值为0。如果是InstanccKlass 或 ArrayKlass，这个值是一个组合数字。
  //(1）对于InstanccKlass而言，组合数字中包含表示对象的以字节为单位的内存占用最。由于instanceklass实例能够表示Java类，因此这里指的内存占用量是指这个Java类创建的对象所需要的内存。
  //(2）对于ArrayKlass而言，组合数字中包含tag、hsize、etype和esize四部分，具体怎么组合和解析由子类实現
  jint        _layout_helper;
  ...
  // ylc
  // 快速查找supertype的一个偏移量，这个偏移量是相对于klass实例的其实地址的偏移量。如果当前类是IOException
  // 那么这个属性就指向_primary_supers数组中存储的IOException的位置。当存储的类多余8个事，值与_secondary_super_cache相等
  juint       _super_check_offset;

  // Class name.  Instance classes: java/lang/String, etc.  Array classes: [I,
  // [Ljava/lang/String;, etc.  Set to zero for all other kinds of classes.
  // ylc
  // 类名，如java/ang/String和Ljava/ang/String;等
  Symbol*     _name;
  ...
  // ylc
  /*
  _primary_supers、_super_check_offset、_secondary_supers与_secondary_super_cache这几个属性完全是为了加快判定父子关系等逻辑而加入的。
  */
  // _primary_supers代表这个类的父类，其类型是一个Klass指针数组，大小固定为8
  // 例如，IOException是Exception的子类，而Exception有时Throwable的子类，因此表示IOException类的——primary_supers属性值为
  // [Throwable,Exception,IOException]。
  // 如果继承链过长，既当前类加上继承的类多余8个(默认值，可通过命令修改)时，会将多出来的类存储到_seconday_supers数组中
  Klass*      _primary_supers[_primary_super_limit]; 
  // java/lang/Class instance mirroring this class
  // oopDesc*类型，保存的是当前Klass实例标识的就Java类所对应的java.lang.Class对象，可以据此访问类的静态属性
  oop       _java_mirror;
  // Superclass
  // Klass指针，指向java类的直接父类
  Klass*      _super;
  // First subclass (NULL if none); _subklass->next_sibling() is next one
  // kalss指针，指向Java类的直接子类，由于直接子类可能有多个，因此多个子类会通过_next_sibling连接起来
  Klass*      _subklass;
  // Sibling link (or NULL); links all subklasses of a klass
  // klass指针，通过该属性可以获取当前类的下一个子类，可通过调用语句
  // _subklass->next_sibling()获取_subklass兄弟子类
  Klass*      _next_sibling;
  ...
  // ClassLoaderData指针，可以通过此属性找到加载该类Java类的ClassLoader
...
protected:
 ...
 public:
  ...
  bool is_subtype_of(Klass* k) const {
    // 当通过_super_check_offset获取的类与k相同时，k存在于当前类的继承链上，肯定有父子关系
    juint    off = k->super_check_offset();
    Klass* sup = *(Klass**)( (address)this + off );
    const juint secondary_offset = in_bytes(secondary_super_cache_offset());
    // 如果k在_primary_supers中，那么利用_primary_supers一定能判断出k与当前类的父子关系
    // 如果k存在于_primary_supers数组中，那么通过_super_check_offset就可快速判断；
    if (sup == k) {
      return true;
    ...
    // 可能有父子关系，需要进一步判断
      ...
      // 如果k存在于_secondary_supers中，需要调用search_secondary_supers()函数来判断
