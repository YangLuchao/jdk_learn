...
Method* Method::allocate(ClassLoaderData* loader_data,
  ...
  // 计算出Method实例需要的内存空间后同样会在元数据区分配内存并调用构造函数创建Method实例
...
Method::Method(ConstMethod* xconst, AccessFlags access_flags, int size) {
  ...
  // Method实例中通过_constMethod属性保存对ConstMethod实例的引用；
  set_constMethod(xconst);
  set_access_flags(access_flags);
  // 通过_method_size属性保存Method实例的大小；
  ...
  // 表示vtable
  // 设置_vtable_index为garbage_vtable_index，表示vtable还不可用
...
int Method::size(bool is_native) {
  ...
  // 如果是本地方法，Method还需要负责保存native_function和signature_handler属性的信息，
  // 因此需要在Method本身占用的内存空间之后再开辟两个指针大小的存储空间， 如图：test/ylctest/6/6.1/表示本地方法的Method实例的内存布局.png
...
void Method::link_method(methodHandle h_method, TRAPS) {
  ...
  // 调用的Interpreter::entry_for_method()函数非常重要，
  // 该函数会根据Java方法类型获取对应的方法执行例程（就是一段机器代码，为方法的调用准备栈帧等必要信息）的入口地址。
  address entry = Interpreter::entry_for_method(h_method);
  assert(entry != NULL, "interpreter entry must be non-null");
  // Sets both _i2i_entry and _from_interpreted_entry
  // 获取执行入口地址后，在Method::link_method()函数中调用set_interpreter_entry()函数为Method设置解释执行的入口
  ...
  // 在Method::link_method()函数中调用make_adapters()函数设置编译执行的入口地址
  (void) make_adapters(h_method, CHECK);

  // ONLY USE the h_method now as make_adapter may have blocked

}
// 调用make_adapters()函数设置编译执行的入口地址
address Method::make_adapters(methodHandle mh, TRAPS) {
  ...
  // 由于HotSpot VM解释模式的调用约定是用栈来传递参数，而编译模式的调用约定更多的是采用寄存器传递参数，二者不兼容，
  // 因而从解释执行中调用已经被编译的方法，或者从编译执行中调用需要解释执行的方法时，都需要在调用时进行适配。
