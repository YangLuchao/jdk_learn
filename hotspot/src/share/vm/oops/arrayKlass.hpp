...
// ArrayKlass类继承自Klass类，是所有数组类的抽象基类
// 数组元素类型（Element Type）指的是数组去掉所有维度的类型
...
class ArrayKlass: public Klass {
 ...
 private:
  ...
  // 指向n+1维的数组
  // Klass指针类型，表示对n+1位数组Klass的引用
  Klass* volatile _higher_dimension;  // Refers the (n+1)'th-dimensional array (if present).
  // 指向n-1维的数组
  // Klass指针类型，表示对n-1维数组Klass的引用
  Klass* volatile _lower_dimension;   // Refers the (n-1)'th-dimensional array (if present).
  // vtable的大小
  // int类型，保存虚函数表的长度
  /*
  _vtable_len的值为5，因为数组是引用类型，父类为Object类，而Object类中有5个虚方法可被继承和重写
  void          finalize()
