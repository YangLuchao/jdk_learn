...
// this klass.
// OopMapBlock是一个简单的内嵌在Klass里面的数据结构，用来描述oop中包含的引用类型属性，
// 即该oop所引用的其他oop在oop中的内存分布，然后就可以根据当前oop的地址找到所有引用的其他oop了
// OopMapBlock表示oop中的引用区域,如图test/ylctest/5/5.5/OopMapBlock表示oop中的引用区域.png
/*
在生成OopMapBlock的过程中调用的函数比较多，处理逻辑比较分散,总结如下：
1、ClassFileParser::parse_fileds():获取对象类型字段信息
2、ClassFileParser::layout_fileds():布局对象类型的字段并将相关信息保存到FiledLayoutInfo中
3、ClassFileParser::compute_oop_map_count():计算当前类中需要的OopMapBlack数量
4、InstanceKlass::nonstatic_oop_map_size()和InstanceKlass::allocate_instance_Klass():为OopMapBlack开辟存储空间
5、ClassFileParser::fill_oop_maps():向OopMapBlock中填充信息
创建流程可以看test/ylctest/5/5.5/OopMapBlock的创建流程
*/
class OopMapBlock VALUE_OBJ_CLASS_SPEC {
 ...
 private:
  //_offset表示第一个所引用的oop相对于当前oop地址的偏移量，count表示有count个连续存放的oop。
  int  _offset;
  uint _count;
};
...
class InstanceKlass: public Klass {
 ...
 public:
  // HotSpot VM在解析一个类时会调用InstanceKlass::allocate_instance_klass()函数分配内存，
  // 而分配多大的内存则是通过调用InstanceKlass::size()函数计算出来的
  // 调用InstanceKlass::allocate_instance_klass()函数创建InstanceKlass实例
  ...
  // 这些值主要标注一个类的加载、连接和初始化3个状态，这些已经在虚拟机规范中有明确的规定，参考地址为https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-5.html
  enum ClassState {
    // 已分配(但尚未链接)
    // 已经分配内存，在InstanceKlass的构造函数中通常会将_init_state初始化为这个状态。
    allocated,                          // allocated (but not yet linked)
    // 已装入并插入到类层次结构中(但尚未链接)
    // 表示类已经装载并且已经插入继承体系中，在SystemDictionary::add_to_hierarchy()函数中会更新InstanceKlass的_init_state属性为此状态。
    ...
    // being_initialized、fully_initialized与initialization_error：在类的初始化函数Instance-Klass::initialize_impl()中会用到，
 ...
 protected:
  ...
  // 数组元素为该类型的数组Klass指针。例如，当ObjArrayKlass实例标识的时候数组切元素类型为Object时，表示Object类的InstanceKlass实例的_array_klass就是指向ObjArrayKlass实例的指针
  ...
  // 以该类作为数组组件类型(只的是数组去掉一个维度的类型)的数组名称，如果当前InstanceKlass实例表示Object类，则名称为“[Ljava/lang/Object;”
  Symbol*         _array_name;

  // Number of heapOopSize words used by non-static fields in this klass
  // (including inherited fields but after header_size()).
  // 非静态字段需要占用的内存空间，以字节为单位，在为该InstanceKlass实例表示的Java类所创建对应的java.lang.class对象(使用oop表示)时，会更具此属性的值分配对象内存，在类解析时会事先计算好这个值
  ...
  // OopMapBlock需要占用的内存空间，以字节为单位。OopMapBlock使用<偏移量，数量>描述Java类(InstanceKlass实例)中各个非静态对象（oop）类型的变量在Java对象中的具体位置，这样垃圾回收时就能找到Java对象中引用的其他对象
  // 非静态OopMapBlock：大小等于_nonstatic_oop_map_size。当前类也会继承父类的属性，因此同样可能需要保存父类的OopMapBlock信息，这样当前的Klass实例可能会含有多个OopMapBlock。GC在回收垃圾时，如果遍历某个对象所引用的其他对象，则会依据此信息进行查找。
  ...
  // Java虚函数表（vtable）所占的内存空间，以字节为单位
  // Java vtable：Java虚函数表，大小等于_vtable_len
  int             _vtable_len;           // length of Java vtable (in words)
  // java接口函数表(itable)所占用的内存空间，以字节为单位
  // Java itable：Java接口函数表，大小等于_itable_len
  ...
  // allocate（已分配内存），
  // loaded（读取classes文件信息并加载到内存中），
  // linked(已经过成功链接和校验)，
  // beging_initialized(正在初始化)，
  // full_initialized（已完成初始化）
  // initialization_error（初始化发生错误）
  // 源代码位置：openjdk/hotspot/src/share/vm/oops/instanceKlass.hpp enum ClassState
  u1              _init_state;                    // state of class
  // 引用类型，表示当前InstanceKlass实例的引用类型，可能是强引用、软引用、和弱引用
  ...
  // 保存解救的指针数组，包含_loacl_interface和间接实现接口
  ...
  //     [generic signature index]
  //     ...
  //  类的字段属性，每个字段有6个属性，分别是assess,name_index,sig_index,initial_value_index,low_offset和high_offset
  // 他们组成了一个元组。assess表示访问控制属性，根据name_index可以获取属性名，根据initial_value_index可以获取初始值，根据low_offset与haig_offset可以获取该属性在内存中的偏移量。保存以上所有属性后还可能保存泛型签名信息
  Array<u2>*      _fields;
 ...
 public:
  ...
  // host class
  // 只在匿名类中存在，为了支持JSR 292中的动态语言特性，会给匿名类生成一个host_klass。
  ...
  static int header_size()            { 
    // HeapWordSize在64位系统下的值为8，也就是一个字的大小，同时也是一个非压缩指针占用的空间
  ...
  static int size(int vtable_length, int itable_length,
    ...
    return align_object_size(header_size() +
           ...
           // 为vtable、itable及nonstatic_oop_map分配的内存空间在类解析的过程中会事先计算好，第4章会详细介绍。
  }
  ...
  // 调用size_helper()函数获取oop对象（表示java.lang.Class对象）的值，
  // 这个值是java.lang.Class类中声明的一些属性需要占用的内存空间，紧随其后的就是静态存储区域。
  // 从_layout_helper属性中获取Java对象所需要的内存空间大小
  int size_helper() const {
    // 调用了layout_helper()函数获取Klass类中定义的_layout_helper属性的值，
    // 然后调用了layout_helper_to_size_helper()函数获取对象所需的内存空间
    // 对象占用的内存空间在类解析过程中会计算好并存储到_layout_helper属性中
