...
// HotSpot VM通过Method与ConstMethod类保存方法的元信息。Method用来保存方法中的一些常见信息，如运行时的解释入口和编译入口
// Method类的继承关系看图test/ylctest/6/6.1/Method类的继承关系.png
// Method实例表示一个Java方法，因为一个应用有成千上万个方法，所以保证Method类在内存中的布局紧凑非常重要。
// 为了方便回收垃圾，Method把所有的指针变量和方法都放在了Method内存布局的前面。
class Method : public Metadata {
 friend class VMStructs;
 private:
  // ConstMethod指针，定义在constMethod.hpp文件中，用于表示方法的不可变部分，入方法ID，方法字节码大小，方法名在常量池中的索引等
  ConstMethod*      _constMethod;                // Method read-only data.
  // MethodData指针，在methodData.hpp中定义，用于方法在执行期手机的相关信息，如方法的调用次数、在C1编译期间代码循环次数和阻塞次数、Profile手机的方法性能相关的数据
  // MethodData的结构基础是ProfileData，记录函数运行状态下的数据
  // MethodData分为三个部分，分别是函数类型运行状态下的相关统计数据、参数类型运行状态下的相关统计数据，以及extra扩展区保存的deoptimization的相关数据
  MethodData*       _method_data;
  /*
   MethodCounters指针，在methodCounters.hpp中定义，用于与大量编译、优化相关的技术，例如：
   1、解释器调用次数
   2、解释器执行时由于一场而终止的次数
   3、方法调用次数(method里面有多少次方法调用)
   4、回边个数
   5、Java方法运行过的分层编译的最高层级(由于HotSpot VM中存在分层编译，所以一个方法可能会被编译器编译为不同的层级，_method_counters只会记录运行过的最高层级)
   6、热点方法计数
   7、基于调用频率的热点方法的跟踪统计
  */
  MethodCounters*   _method_counters;
  // AccessFlags类，表示方法的访问控制标识
  // 标识符定义在jvm.hz中如：JVM_ACC_PUBLIC
  AccessFlags       _access_flags;               // Access flags
  // 当前method实例表示的Java方法在vtable表中的索引
  /*
  _vtable_index的取值说明：
  itable_index_max          -10   首个itable的索引
  pending_itable_index      -9    itable将会被赋值
  invalid_vtable_index      -4    无效的虚表index
  garbage_vtable_index      -3    还没有初始化的vtable方法，无用值
  nonvirtual_vtable_index   -2    不需要虚函数派发，比如调用静态法方法
  */
  int               _vtable_index;               // vtable index of this method (see VtableIndexFlag)
                                                 // note: can have vtables with >2**16 elements (because of inheritance)
  // 当前method实例的大小，以字为单位
  u2                _method_size;                // size of this object
  // 固有方法的ID。固有方法（Intrinsic Method）在Hotspot VM中表示一些众所周知的方法，针对他们可以做特别处理，生成独特的代码例程， 
  // Hotspot VM发现一个方法是固有方法时不会对字节码进行解释执行，而是调到独特的代码例程上执行，这要比解释执行更高效
  ...
  // 指向该Java方法的签名（signature）所对应的i2c2i adapter stub。当需要c2i adapter stub或i2c adapter stub的时候，调用_adapter的get_i2c_entry()或get_c2i_entry()函数获取
  ...
  // 一开始Java方法没有被JIT编译，需要在解释模式下执行。
  // 当该方法被JIT编译并“安装”完成之后，_from_compiled_entry会指向编译出来的机器码入口，具体说是指向verified entry point。
  // 如果要抛弃之前编译好的机器码，那么_from_compiled_entry会恢复为指向c2i adapter stub。
  ...
  // _from_interpreted_entry的初始值与_i2i_entry一样，都是指向字节码解释执行的入口。
  // 但当该Java方法被JIT编译并“安装”之后，_from_interpreted_entry就会被设置为指向i2c adapter stub。
  // 如果因为某些原因需要抛弃之前已经编译并安装好的机器码，则_from_interpreted_entry会恢复为指向_i2i_entry。
  // 如果有_code，则通过_from_interpreted_entry转向编译方法，否则通过_i2i_entry转向解释方法
  volatile address           _from_interpreted_entry; // Cache of _code ? _adapter->i2c_entry() : _i2i_entry
  // 以上5个属性对于Java方法的解释执行和编译执行非常重要
