...
// java.lang.ref.Reference类需要使用C++类InstanceRefKlass的实例来表示，
// 在创建这个类的实例时，_reference_type字段（定义在InstanceKlass类中）的值通常会指明java.lang.ref.Reference类表示的是哪种引用类型
// 表示Java引用类型的InstanceRefKlass
// 当为引用类型但是又不是REF_SOFT、REF_WEAK、REF_FINAL和REF_PHANTOM中的任何一种类型时，_reference_type属性的值为REF_OTHER。
