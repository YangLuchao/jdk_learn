...
class CallInfo;
// ConstantPoolCacheEntry用于缓存具体的信息
class ConstantPoolCacheEntry VALUE_OBJ_CLASS_SPEC {
 ...
 private:
  /*
  以上类中定义的4个字段能够传达非常多的信息。它们的长度相同，以32位为例来介绍这4个字段。
  如果当前的ConstantPoolCacheEntry表示的是字段入口，则这几个字段的信息如图 test/ylctest/7/7.3/表示字段的ConstantPoolCacheEntry的字段信息.png
  如果当前的ConstantPoolCacheEntry表示的是方法入口，则这几个字段的信息如图 test/ylctest/7/7.3/表示方法的ConstantPoolCacheEntry的字段信息.png
...
（4）invokestatic：调用静态方法，不需要分发机制。
  ...
  在invokevirtual和invokespecial等字节码指令对应的汇编片段中，如果_indices中的invoke code for _f1或invoke code for_f2不是字节码指令的操作码，说明方法还没有连接，
  需要调用InterpreterRuntime::resolve_invoke()函数连接方法，同时为ConstantPoolCacheEntry中的各属性生成相关的信息。
  */
...
// ConstantPoolCache类里保存了连接过程中的一些信息，从而让程序在解释执行的过程中避免重复执行连接过程
class ConstantPoolCache: public MetaspaceObj {
  friend class VMStructs;
  friend class MetadataFactory;
 private:
  // _length表示Rewriter类中_cp_cache_map整数栈中的数量
  int             _length;
  // _constant_pool表示ConstantPoolCache保存的是哪个常量池的信息。
  // ConstantPoolCacheEntry用于缓存具体的信息，在内存中的布局就是一个ConstantPoolCache后紧跟着数个ConstantPoolCacheEntry。
  // 这样size()和base()等函数的实现逻辑就不难理解了。
 ...
 private:
  ...
  // // 返回的是字的数量ConstantPoolCache加上length个ConstantPoolCacheEntry的大小,in_words(ConstantPoolCacheEntry::size())的值为4，表示每个ConstantPoolCacheEntry需要占用4字节
