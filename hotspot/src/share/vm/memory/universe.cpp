...
// Known objects
// _charArrayKlassObj属性保存的是表示boolean数组的TypeArrayKlass实例，
// 该实例是通过调用TypeArrayKlass::create_klass()函数创建的，
...
// Universe::genesis()函数中有对数组及核心类的加载逻辑。
// 数组类没有对应的Class文件，因此在类加载阶段，基本类型的一维数组会被HotSpot VM直接创建，并且不需要进行验证、准备和初始化等操作。
// 类加载就是通过宏来定义一些需要加载的核心类，然后调用前面介绍的一些类加载器方法来加载类。
/*
...
void Universe::genesis(TRAPS) {
  ...
  { FlagSetting fs(_bootstrapping, true);
    ...
    { MutexLocker mc(Compile_lock);
      ...
      if (!UseSharedSpaces) {
        // HotSpot VM在初始化时就会创建Java中8个基本类型的一维数组实例TypeArrayKlass。前面在介绍HotSpot VM启动时讲过initializeJVM()函数，
        // 这个函数会调用Universe::genesis()函数，在Universe::genesis()函数中初始化基本类型的一维数组实例TypeArrayKlass
        // 有了元素类型为基本类型的一维数组后，可以方便地创建出多维数组
        // 对于对象类型数组而言，只要创建代表对象类型的InstanceKlass实例，就可以根据InstanceKlass及ObjArrayKlass中的一些字段表示n维对象类型数组
        ...
        // 创建出来的一维数组会被存储到类型为Klass*的typeArrayKlassObjs数组中，这样就可以根据这些一维数组的TypeArrayKlass实例创建出多维数组了。
        // 前面介绍java_lang_Class::create_basic_type_mirror()函数时曾调用了typeArrayKlassObj()函数，其实就是获取创建出来的一维数组的实例。
  ...
  // HotSpot在Universe::genesis()函数中创建Object数组
  // 
  _objectArrayKlassObj = InstanceKlass::
    cast(SystemDictionary::Object_klass())
    // 调用表示Object类的InstanceKlass类的array_klass()函数
    // 传递的参数1表示创建Object的一维数组类型
    ->array_klass(1, CHECK);
  // OLD
...
// 基本类型的mirror值在HotSpot VM启动时就会创建
...
// Universe::preferred_heap_base()函数主要是处理压缩指针的地址，我们暂不研究压缩指针的情况
// 所以为虚拟机加了-XX:-UseCompressedOops选项，此函数最终会返回addr，表示对堆的基址没有任何要求
...
/*
在虚拟机启动时会调用initialize_heap()函数初始化垃圾收集相关的参数。调用链如下：
...
jint Universe::initialize_heap() {
  ...
  if (UseParallelGC) {
    ...
    fatal("UseConcMarkSweepGC not supported in this VM.");
      ...
      // 针对不同的垃圾收集器，初始化逻辑也不同。我们只针对Serial收集器，因此只看这部分逻辑即可。
      // 其中涉及GenCollectorPolicy类型变量gc_policy的初始化，初始化为MarkSweepPolicy回收策略。
      gc_policy = new MarkSweepPolicy();
    }
    // 初始化MarkSweepPolicy::_generations属性如0为Generation::DefNew，1为Generation::MarkSweepCompact
    gc_policy->initialize_all();
    // 由于MarkSweepPolicy类中并没有实现initialize_all()函数，所以最终调用GenCollectorPolicy::initialize_all()函数进行策略初始化。
    // 堆使用GenCollectedHeap来管理
    Universe::_collectedHeap = new GenCollectedHeap(gc_policy);
  }

  ThreadLocalAllocBuffer::set_max_size(Universe::heap()->max_tlab_size());
  // Universe::heap()函数获取的就是Universe::_collectedHeap属性，调用initialize()函数初始化堆
...
// 调用Universe::reserve_heap()函数按年轻代和老年代要求的最大内存值分配虚拟空间
ReservedSpace Universe::reserve_heap(size_t heap_size, size_t alignment) {
  ...
  // 通过ReservedHeapSpace来分配一段逻辑上连续的内存地址范围，ReserveHeapSpace继承自ReserveSpace类，在ReservedHeapSpace类中没有声明任何属性
