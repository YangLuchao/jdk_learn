...
class oopFactory: AllStatic {
 public:
  // 当需要创建typeArrayOopDesc实例时，通常会调用oopFactory类中定义的工厂方法
  // Basic type leaf array allocation
  // 代码中调用Universe::boolArrayKlassObj()函数获取_charArrayKlassObj属性的值。
  ...
  // objArrayOop的创建与typeArrayOop的创建非常类似，
  // 即先调用oopFactory类中的工厂方法new_objectArray()，然后调用ObjArrayKlass::allocate()函数分配内存
