...
// 首先对保存着年轻代和老年代的GenerationSpec实例中的_init_size和_max_size变量的值进行对齐操作，
// 然后调用allocate()函数为堆分配内存，有了内存后就可以通过GenerationSpec::init()
// 创建表示年轻代的DefNewGenereation和表示老年代的TenuredGeneration实例了
jint GenCollectedHeap::initialize() {
  ...
  // 调用的是TwoGenerationCollectorPolicy类中定义的number_of_generations()函数，此函数返回常量值2，表示共有两个代，即年轻代和老年代
  ...
  for (i = 0; i < _n_gens; i++) {
    ...
    // 调用GenerationSpec::init()函数返回Generation实例将这个实例存储到_gen数组中
...
// 调用allocate()函数为堆分配内存
// 调用链如图 test/ylctest/8/8.3/分配内存的调用链.png 所示
