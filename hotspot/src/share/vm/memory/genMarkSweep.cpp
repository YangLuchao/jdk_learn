...
void GenMarkSweep::mark_sweep_phase1(int level,
  ...
  // 在GenMarkSweep::mark_sweep_phase1()函数中标记完堆中所有的活跃对象后就能确定哪些类加载器已经失效了，
  // 此时会在调用的SystemDictionary::do_unloading()函数中调用ClassLoaderDataGraph::do_unloading()函数找出失效的类加载器，
  // 然后通过定义在ClassLoaderDataGraph类中的_unloading静态属性保存，多个失效的类加载器会形成一个单链表。
