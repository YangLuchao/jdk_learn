    ...
    _should_allocate_from_space(false)
{
  ...
  // 构造函数会创建对应Eden空间和两个Survivor空间的实例并通过对应的变量进行保存。
  ...
  _next_gen = NULL;
  // 控制新生代对象晋升到老年代中的最大阈值
  // _tenuring_threshold为对象的晋升阈值，如果某个对象经历过_tenuring_threshold次GC后依然存活，则可以晋升到老年代
  _tenuring_threshold = MaxTenuringThreshold;
  // 当新对象申请的内存空间大于这个参数值的时候，直接在老年代中分配内存
  // _pretenure_size_threshold_words保存-XX:PretenureSize-Threshold命令的值，意思是超过这个值时对象直接在old区分配内存。默认值是0，意思是不管值多大，都是先在Eden中分配内存。
  _pretenure_size_threshold_words = PretenureSizeThreshold >> LogHeapWordSize;

  _gc_timer = new (ResourceObj::C_HEAP, mtGC) STWGCTimer();
}
// 调用compute_space_boundaries()函数计算Eden和两个Survivor空间的边界
void DefNewGeneration::compute_space_boundaries(uintx minimum_eden_size,
  ...
  // Compute sizes
  // 计算Eden和两个Survivor空间的值，注意在获取内存时调用的是committed_size()函数也就是实际上获取的是已经分配的物理内存
  ...
  // 调用了initialize()函数初始化Eden和两个Survivor空间
