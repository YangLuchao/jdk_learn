...
class Metabase VALUE_OBJ_CLASS_SPEC {
  // 通过_next与_prev属性将Metachunk块连接成双链表，根据_word_size可以计算出Metachunk块的大小。
  size_t _word_size;
  T*     _next;
  T*     _prev;

 protected:
  //Metabase和Metachunk中还定义了一些常见的函数，调用这些函数可以获取Metachunk的一些信息，如调用free_word_size()函数可以获取空闲内存大小。
...
// Metachunk的内存结构如 test/ylctest/8/8.2/Metachunk块的结构.png所示
