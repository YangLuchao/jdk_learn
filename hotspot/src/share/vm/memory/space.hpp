...
// Generation类的实现是基于Space类的，Space类负责实际的内存管理，
// Space类的继承关系如图 test/ylctest/8/8.3/Space类的继承关系.png
// Space是内存区间的基类，支持内存分配、内存空间计算和垃圾回收
// 采用采用Serial收集器回收年轻代堆空间时会采用复制算法，与复制算法相关的变量定义如图 test/ylctest/8/8.3/与复制算法相关的变量.png 所示
// 各个变量指向如 test/ylctest/8/8.3/复制算法中各变量的指向.png 所示
class Space: public CHeapObj<mtGC> {
  friend class VMStructs;
 protected:
  // 其中，_bottom和_end属性指向Space实例表示的内存区间的首地址和尾地址。为了方便内存管理，首地址和尾地址都指向内存页的边界，假设Space实例为两个内存页的大小
  // 指向如图 test/ylctest/8/8.3/_bottom和_end属性描述.png
  HeapWord* _bottom;
  HeapWord* _end;

  // Used in support of save_marks()
  // _saved_mark_word属性辅助进行垃圾回收
...
// ompactibleSpace类继承自Space类，增加了压缩操作，即在垃圾回收后通过复制并移动Java对象的位置来减少Space内部的内存空白和碎片问题，提升内存利用率。
...
// ContiguousSpace表示空间是连接的，这样就能支持快速地进行内存分配和压缩操作。
class ContiguousSpace: public CompactibleSpace {
  friend class OneContigSpaceCardGeneration;
  friend class VMStructs;
 protected:
  // _top属性保存Space实例表示的内存空间的起始地址，在分配内存时会从这个字段指向的地址开始分配。
  HeapWord* _top;
...
// EdenSpace类继承自ContiguousSpace类，用来表示Eden空间
class EdenSpace : public ContiguousSpace {
  friend class VMStructs;
 private:
  // _gen属性保存了此Eden区所归属的代，类型为DefNewGeneration，表示归属年轻代。
