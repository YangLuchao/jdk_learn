...
protected:
  ...
  // gen_size.
  // 调用compute_survivor_size()函数计算Survivor的大小
  size_t compute_survivor_size(size_t gen_size, size_t alignment) const {
    // 在JVM参数中有一个比较重要的参数SurvivorRatio，用于定义新生代中Eden空间和Survivor空间（From Survivor空间或To Survivor空间）的比例，默认为8。
    // 也就是说，Eden空间占新生代的8/10，From Survivor空间和To Survivor空间各占新生代的1/10。
