...
// 抽象概念 代 的实现
// Generation 的继承关系如图 test/ylctest/8/8.3/Generation类的继承关系.png
/*
...
*/
// 不同的类型对应的Generation及GenCollectedHeap类的继承体系如图 test/ylctest/8/8.3/不同的类型对应的Generation及GenCollectedHeap类的继承体系.png
...
class CardGeneration: public Generation {
  friend class VMStructs;
 protected:
  // 定义了一些与卡表实现相关的属性，如记忆集和偏移表。
  // 记忆集用来记录老年代到年轻代堆空间的引用信息，而偏移表用来记录卡表对应的一块内存中对象的起始位置。
