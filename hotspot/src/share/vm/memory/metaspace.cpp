...
// ChunkManager类管理着所有类加载器卸载后释放的内存块Metachunk。
class ChunkManager : public CHeapObj<mtInternal> {
  friend class TestVirtualSpaceNodeTest;

  // ChunkManager类中的_free_chunks属性类似于SpaceManager类中的_chunks_in_use属性，
  // 但是会通过Freelist管理Metachunk，并且不管理超大块，超大块由_humongous_dictionary管理。
  // 因为ChunkManager类管理的空闲块有频繁的查询请求，几乎每次内存分配都要先从这些空闲块开始查询、分配，所以组织了高效的查询数据结构。
...
class SpaceManager : public CHeapObj<mtClass> {
 ...
 private:
  ...
  // _chunks_in_use数组保存所有当前类加载器使用的Metachunk块
  // NumberOfInUseLists枚举看定义
  // SpaceManager类定义中的_chunks_in_use是一个存储4个Metachunk*类型的数组，每个槽上存储一个链表结构，链表结构中的Metachunk块大小相同。
  Metachunk* _chunks_in_use[NumberOfInUseLists];
  // 当前用来分配内存的Metachunk块由_current_chunk来保存
  ...
  // _block_freelists属性，它也会保存许多空闲的内存块，为了加快查找速度，这些块底层使用了字典结构。这些内存块并不是一整个Metachunk，也不是为了复用Metachunk块，而是为了充分复用Metachunk块的剩余空间。
  // 例如，当前正在负责为类加载器分配内存的Metachunk块的剩余空间不够大，这次的分配请求超过了剩余的内存空间，那只能从管理空闲块的ChunkManager中申请空闲块，
  // 或从VirtualSpaceNode中声明一个新的空闲块，然后在新的空闲块中分配。
  // 这样会带来一个问题，即原Metachunk块中剩余的空间会被浪费，为了充分利用这部分空间，
  // 只能将剩余空间作为一个小的空闲块保存到_block_freelists中，这样下次申请分配相对较小的内存时直接从_block_freelists中分配即可。
...
// 遍历VirtualSpaceNodes列表并删除container_count为0的节点。将节点中的Metachunks从它们各自的自由列表中移除。
// 当一个VirtualSpaceListNode中的所有chunk块都处于空闲状态的时候，这个VirtualSpaceListNode就会从链表VirtualSpaceList中移除，
// VirtualSpaceList中的所有chunk块也会从空闲列表中移除。
// 此时这个VirtualSpaceListNode就不会再使用了，其内存会归还给操作系统。
// 对于一个空闲的VirtualSpaceListNode来说，拥有此空闲VirtualSpaceListNode中的chunk块的所有类加载器必然都已经被卸载了。
// 至于是否能让VirtualSpaceListNode中的所有chunk块都空闲，主要取决于碎片化的程度。
// 一个VirtualSpaceListNode的大小是2MB，chunk块的大小为1KB、4KB或64KB，因此一个VirtualSpaceListNode上通常有约150~200个chunk块。
// 如果这些chunk块全部归属于同一个类加载器，那么回收这个类加载器就可以一次性回收这个VirtualSpace-ListNode，并且可以将其空间还给操作系统。
// 如果将这些chunk块分配给不同的类加载器，每个类加载器都有不同的生命周期，那么什么都不会被释放。这是在告诉我们要小心对待大量的小的类加载器，如那些负责加载匿名类或反射类的加载器。
...
// 根据当前要求分配的内存容量word_size计算分配哪个内存块Metachunk，前面介绍过4种不同大小的内存块。
...
MetaWord* SpaceManager::grow_and_allocate(size_t word_size) {
  ...
  // 调用calc_chunk_size()函数根据当前要求分配的内存容量word_size计算分配哪个内存块Metachunk，前面介绍过4种不同大小的内存块。
  size_t chunk_word_size = calc_chunk_size(word_size);
  // 计算好后就调用get_new_chunk()函数获取块
...
SpaceManager::~SpaceManager() {
  ...
  // 将SpaceManager中使用的所有内存块交给ChunkManger来管理，Chunk-Manger中的空闲列表是一个全局的空闲列表，
  // 任何SpaceManager需要新块时都可以从ChunkManger中获取。
...
// SpaceManager分配元空间内存，并进行管理
MetaWord* SpaceManager::allocate(size_t word_size) {
  MutexLockerEx cl(lock(), Mutex::_no_safepoint_check_flag);

  size_t raw_word_size = get_raw_word_size(word_size);
  // 对于SpaceManager类中定义的类型为BlockFreelist的_block_freelists在前面介绍过，当某次请求的内存大小无法从当前的Metachunk块中分配时，
  // 如果当前的Metachunk块剩余的空闲空间相对比较大，就会将这个Metachunk块保存到_block_freelists中，
  // 当下一次分配请求到来时，如果请求的内存比较小，这个块中剩余的空闲空间还能进行分配内存的操作。
...
// 调用Metachunk的allocate()函数从当前块中分配内存，如果分配失败，
// 调用grow_and_allocate()函数从新的块中分配（从空闲块中找到合适的新块或从VirtualSpaceNode中分配新块）内存。
...
MetaWord* Metaspace::allocate(size_t word_size, MetadataType mdtype) {
  ...
  // Metaspace中定义的SpaceManager类型的_class_vsm和_vsm都是实例变量，也就是每个类加载器都对应着一个SpaceManager实例，管理着为自己分配的Metachunk块
...
// 元空间分配内存
// Metaspace::allocate()函数会从元空间区分配内存，如果分配失败可能还会触发垃圾回收，分配完内存后还会对内存执行清零操作。
MetaWord* Metaspace::allocate(ClassLoaderData* loader_data, size_t word_size,
  ...
  // 在分配元数据区时，首先要调用类加载器的metaspace_non_null()函数获取Metaspace实例
