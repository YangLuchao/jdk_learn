...
void CollectorPolicy::initialize_flags() {
  ...
  // 为了防止频繁扩展内存代空间，每次扩展内存时都有一个最小值_min_heap_delta_bytes（在CardGeneration类中声明的变量），由命令-XX:MinHeapDeltaBytes指定，x86架构下的默认值为128KB。
  ...
  // 设置-Xmx与-Xms
  // InitialHeapSize与MaxHeapSize可以通过-XX:InitialHeapSize与-XX:+MaxHeap-Size命令指定，_min_heap_byte_size属性的值也可以通过-Xms命令指定。
  // 初始堆、最小堆及最大堆的计算
...
size_t CollectorPolicy::compute_heap_alignment() {
  ...
  // 调用GenRemSet::max_alignment_constrain()函数计算alignment的值
...
// 首先计算堆的最小值，这个值的基本要求是能容纳两个代
size_t GenCollectorPolicy::young_gen_size_lower_bound() {
  // The young generation must be aligned and have room for eden + two survivors
  // 对于年轻代来说，有Eden和两个Survivor空间，因此至少要求3*_space_alignment的空间，对齐到代后就是年轻代的最小值
...
void GenCollectorPolicy::initialize_flags() {
  ...
  // 老年代的最小空间同样是_space_alignment，对齐到代后就求出了老年代的最小值，将年轻代和老年代的最小值相加后对齐到_heap_alignment后求出了堆的最小值。
  ...
  // 由于-XX:NewSize选项指定了新生代初始内存的大小，所以最终堆的大小要取二者的最大值
...
// TwoGenerationCollectorPolicy::initialize_flags()函数会确定老年代的内存空间。
// 在确定老年代的内存空间时会结合-XX:OldSize与-XX:NewSize等选项进行判断，最终将得到的值保存在_min_gen1_size、_initial_gen1_size和_max_gen1_size变量中。
...
// 结合NewSize、MaxNewSize和NewRatio变量的值决定年轻代的最大值、初始值和最小值，
// 不过计算出来的值还要综合考虑整个堆的最大值、初始值和最小值，同时要保证年轻代的最小值≤年轻代的初始值≤年轻代的最大值
...
//如果要调整老年代，则说明老年代在使用_min_gen1_size时不产生冲突，有调整的余地，或者也可能是年轻代已经调整到了最小值smallest_new_size，没有可调整的余地，只能调整老年代。
...
MetaWord* CollectorPolicy::satisfy_failed_metadata_allocation(
  ...
  do {
    ...
    // 调用VMThread::execute()函数将任务放入队列中，由专门的垃圾回收线程执行任务，
    // 当前的线程会等待最终的分配结果
    VMThread::execute(&op);
...
// 由以上代码可知，内存代和内存空间的地址要按2的16次方大小对齐，而堆也要对齐，通过compute_heap_alignment()函数计算对齐的数值
  _heap_alignment = compute_heap_alignment();
}
// 调用initialize_generations()函数可以初始化内存代的逻辑
// 使用_generations保存年轻代和老年代的相关信息，这里并没有创建表示年轻代的DefNewGeneration实例和表示老年代的TenuredGeneration实例
