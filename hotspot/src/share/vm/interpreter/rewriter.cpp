...
// compute_index_maps()函数可以生成常量池缓存项索引，但同时要保证常量池项索引和常量池缓存项索引之间的映射关系
// 常量池项索引和常量池存缓存项的关系如图：test/ylctest/7/7.3/常量池项与常量池缓存项.png
// 对于某些使用常量池索引作为操作数的字节码指令来说，当重写字节码指令后，原常量池索引会更改为指向常量池缓存项的索引
// 之所以创建常量池缓存，部分原因是这些指令所需要引用的信息无法使用一个常量池项来表示，
   ...
   u2 name_and_type_index;
      ...
      */
      ...
      case JVM_CONSTANT_Methodref         : // fall through
        ...
        break;
      /*
在Rewriter::compute_index_maps()函数中调用add_resolved_references_entry()函数，
为CONSTANT_String_info、CONSTANT_MethodHandle与CONSTANT_MethodType项生成已经解析过的常量池项索引。
这里只介绍CONSTANT_String_info，另外两个常量池项是为了让Java语言支持动态语言特性而在OpenJDK 7版本中新增的常量池项，这里不做介绍。 
CONSTANT_String_info项的格式如下：
 CONSTANT_String_info {
  ...
  u2 string_index;            // 指向字符串字面量的索引，是CONSTANT_Utf8_info项
      ...
      */
      ...
      case JVM_CONSTANT_MethodType        : // fall through
        // add_resolved_references_entry()函数的实现与add_cp_cache_entry()类似
  ...
  // 更新变量_resolved_reference_limit与_first_iteration_cp_cache_limit的值
  record_map_limits();
...
void Rewriter::make_constant_pool_cache(TRAPS) {
  ...
  ConstantPoolCache* cache =
      // 根据Rewriter类中的_cp_cache_map和_invokedynamic_cp_cache_map等变量信息计算创建ConstantPoolCache实例需要占用的内存空间。
...
// 重写object构造方法
// 可能将构造方法的return指令改写为Bytecodes::_return_register_finalizer，这是HotSpot VM内部使用的扩展指令，方便后续处理该指令时调用Finalizer.register()方法注册Finalizer对象
...
// 调用Rewriter::maybe_rewrite_ldc()函数可能会重写ldc指令和指令的操作数，也就是常量池索引
/*
ldc指令从常量池中取值然后压入栈中。如果ldc操作数索引到的常量池项为CONSTANT_String_info、CONSTANT_MethodHandle_info或CONSTANT_MethodType_info，
则重写ldc字节码指令为HotSpot VM中的扩展指令_fast_aldc_w或_fast_aldc，同时将操作数改写为对应的常量池缓存索引。
*/
...
// 把某些指令的操作数从常量池下标改写为常量池缓存下标。
// 之所以创建常量池缓存，部分原因是这些指令所需要引用的信息无法使用一个常量池项来表示，而需要使用一个更大的数据结构表示常量池项的内容，
// 另外也是为了不破坏原有的常量池信息
// 在Rewriter::scan_method()函数中循环当前方法的所有字节码，然后对需要重写的字节码指令进行重写
void Rewriter::scan_method(Method* method, bool reverse, bool* invokespecial_error) {
...

  {
    ...
    // 获取字节码指令的长度，有些字节码指令的长度无法通过length_for()函数来计算因此会返回0，需要进一步调用length_at()函数来获取
    int bc_length;
