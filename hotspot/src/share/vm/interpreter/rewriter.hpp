...
class Rewriter: public StackObj {
 private:
  ...
  void init_maps(int length) {
    ...
    // 因为并不是所有的常量池项都需要生成常量池项索引，向栈中压入常量池项后生成常量池缓存项的索引，通过常量池索引项可以找到常量池项索引
  ...
  // 新增映射实例
  // 通过cp_cache_map和cp_map建立了cp_index与cache_index的对应关系
  // 实例如 test/ylctest/7/7.3/7-1
  ...
  // 注意，对于重写的invokespecial来说，7.3.1节并没有为invokespecial指令创建cp_index与cache_index，
  // 因此最终会通过_cp_cache_map压入cp_index来获取一个新的cache_index。
