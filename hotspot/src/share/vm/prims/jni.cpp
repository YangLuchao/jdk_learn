...
// jni执行静态方法
// 最终通过JavaCalls::call()函数调用Java方法，
static void jni_invoke_static(JNIEnv *env, JavaValue* result, jobject receiver, JNICallType call_type, jmethodID method_id, JNI_ArgumentPusher *args, TRAPS) {
 
  // jni_invoke_static()函数看起来比较复杂，是因为JNI调用时需要对参数进行转换，在JNI环境下只能使用句柄访问HotSpot VM中的实例，
  // 因此在每次函数的开始和结束时都需要调用相关函数对参数进行转换，
  // 如调用Method::resolve_jmethod_id()、调用JNIHandles::make_local()等函数。
...
static jmethodID get_method_id(JNIEnv *env, jclass clazz, const char *name_str,
  ...
  if (name == vmSymbols::object_initializer_name() ||
    ...
    if (klass->oop_is_instance()) {
      // 在查找构造方法时调用了InstanceKlass类中的find_method()函数，这个函数不会查找超类；
    ...
    // 在特定类中查找方法
    // 在查找普通方法时调用了Klass类中的lookup_method()
...
// jni获取methodId
// 传递的参数name为"checkAndLoadMain"，而sig为"(ZILjava/lang/String;)Ljava/lang/Class;"，也就是checkAndLoadMain()方法的签名
