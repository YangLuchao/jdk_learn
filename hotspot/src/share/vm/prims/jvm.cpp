...
// 通过forName()方法调用find_class_from_class_loader()函数时，还会调用initialize()函数执行类的初始化操作
...
// JVM_ENTRY是宏定义，用于处理JNI函数调用的预处理，如获取当前线程的JavaThread指针。
// 因为垃圾回收等原因，JNI函数不能直接访问Klass和oop实例，只能借助jobject和jclass等来访问，
// 所以会调用JNIHandles::resolve_non_null()、JNIHandles::resolve()与JNIHandles::mark_local()等函数进行转换。
...
jclass find_class_from_class_loader(JNIEnv* env, Symbol* name, jboolean init,
  ...
  // init的值为true klass_handle->initialize(CHECK_NULL);
  // 对类进行初始化操作
