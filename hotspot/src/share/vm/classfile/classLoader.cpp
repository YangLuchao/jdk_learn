...
// 加载类
// 使用启动类加载器，那么可能需要调用load_classfile()函数装载类。
instanceKlassHandle ClassLoader::load_classfile(Symbol* h_name, TRAPS) {
  ...
  instanceKlassHandle h;
  {
    ...
    // 循环遍历ClassPathEntry链表结构，直到查找到某个类路径下名称为name的文件为止，
    // 此时open_stream()函数会返回名称为name的Class文件的ClassFileStream实例，
  ...
  if (stream != NULL) {
    ...
    // 类文件解析的入口是ClassFileParser类中定义的parseClassFile()函数
    // 对Class文件进行解析
