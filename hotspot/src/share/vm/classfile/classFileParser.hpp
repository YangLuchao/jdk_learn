...
// 每个Class文件都对应唯一一个类或接口的定义信息，但类或接口不一定定义在Class文件里（类或接口通过类加载器直接生成）。
// 每个Class文件都可以看作是由字节流组成的，可按照1、2、4、8个字节为单位进行读取。多字节数据项是按照Big-Endian的顺序进行存储的。
// 项item用来描述类结构格式的内容，在Class文件中，各项按照严格的顺序连续存放，各项之间无任何填充或分割符。
...
// 在类解析的过程中，解析出的信息会暂时保存在ClassFileParser实例的相关变量中，最后会创建InstanceKlass实例保存这些信息，然后将InstanceKlass实例放入字典中，在保证唯一性的同时也提高了查询效率。
