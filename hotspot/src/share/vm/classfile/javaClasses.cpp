...
// 计算出这几个变量对应的字段在oop（表示java.lang.Class对象）实例中的偏移量
...
#undef LOOKUP_INJECTED_FIELD
  // 如果当前的类是java.lang.Class，那么最终count的值为7，表示有7个字段要注入，而start变量的值为java_lang_Class_klass_enum。
  if (start != -1) {
    *field_count = count;
    // 有了start值后，可以从_injected_fields数组中查找InjectedField
    // 数组加数字，取start索引上的值
...
static void initialize_static_field(fieldDescriptor* fd, Handle mirror, TRAPS) {
  ...
  if (fd->has_initial_value()) {
    ...
    // 获取定义当前静态字段的InstanceKlass实例，通过_java_mirror找到oop实例，
    // 然后在oop实例的对应槽位上存储初始值，具体存储在哪里，
    // 是通过调用fieldDescriptor类的offset()函数获取的。
    // 无论是静态字段还是非静态字段，都需要计算具体的布局，也就是计算每个字段存储在实例的哪个位置上。
    switch (t) {
      case T_BYTE:
...
void java_lang_Class::initialize_mirror_fields(KlassHandle k,
  ...
  // do_local_static_fields()函数会对静态字段进行初始化注意此时传入的是initialize_static_field()函数指针
  InstanceKlass::cast(k())->do_local_static_fields(&initialize_static_field, mirror, CHECK);
}

// InstanceKlass实例保存了Class文件解析的结果，每个InstanceKlass实例还会有java.lang.Class对象，通过oop实例来表示，因此在设置完InstanceKlass实例的相关属性后，会调用create_mirror()函数创建oop实例
// java_lang_Class::create_mirror()函数对_component_mirror属性进行设置
// 一维或多维数组的元素类型如果是对象，使用Klass实例表示，如Object[]的元素类型为Object，使用InstanceKlass实例表示；
// 一维或多维数组的元素类型如果是基本类型，因为没有对应的Klass实例，所以使用java.lang.Class对象描述boolean和int等类型，
// 这样基本类型的数组就会与oop对象（表示java.lang.Class对象）产生关联
void java_lang_Class::create_mirror(KlassHandle k, Handle class_loader,
  ...
  if (SystemDictionary::Class_klass_loaded()) {
    ...
    // 最后创建oop实例，不过在分配内存空间时，会考虑静态变量，所以说Java类的静态变量存储在java.lang.Class对象中
    ...
    // oop实例和InstanceKlass实例引用关系看图：test/ylctest/4/4.1
    if (!k.is_null()) {
      // 在oop实例的某个偏移位置存放指定InstanceKlass的指针
      // 指向InstanceKlass实例的_klass_offset字段的设置
...
oop java_lang_Class::create_basic_type_mirror(const char* basic_type_name, BasicType type, TRAPS) {
  ...
  // 调用InstanceMirrorKlass实例（表示java.lang.Class类）的allocate_instance()函数创建oop（表示java.lang.Class对象）
  // _component_mirror最终设置的就是这个oop。
...
// compute_offsets()函数会在Class文件解析后调用，计算出这几个变量对应的字段在oop（表示java.lang.Class对象）实例中的偏移量，
void java_lang_Class::compute_offsets() {
  ...
  java_lang_Class::_klass_offset                  =
   JavaClasses::compute_injected_offset(JavaClasses::java_lang_Class_
klass_enum);
...
oop_size_enum);
  java_lang_Class::_static_oop_field_count_offset =
   JavaClasses::compute_injected_offset(JavaClasses::java_lang_Class_
static_oop_field_count_enum);
  java_lang_Class::_protection_domain_offset      =
   JavaClasses::compute_injected_offset(JavaClasses::java_lang_Class_
protection_domain_enum);
...
#endif // PRODUCT
// 计算出这几个变量对应的字段在oop（表示java.lang.Class对象）实例中的偏移量
// 获取注入字段在oop实例中的偏移量并通过相关变量保存这个偏移量。
// 这样在得到java.lang.Class对象相关字段的值后就可以利用偏移量直接找到对应的内存存储位置，
// 在这个位置上存储相关的字段值
