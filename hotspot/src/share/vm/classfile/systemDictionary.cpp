...
// HotSpot VM在启动时会初始化一个重要的变量
// _java_system_loader属性用于保存应用类加载器实例，HotSpotVM在加载主类时会使用应用类加载器加载主类。
// _java_system_loader属性用于在compute_java_system_loader()函数中进行初始化
/*
调用链如下
JavaMain()                                      java.c
...
// 通过AppClassLoader对象中的parent属性获取ExtClassLoader对象
...
void SystemDictionary::compute_java_system_loader(TRAPS) {
  ...
  // JavaClass::call_static()函数调用了java.lang.ClassLoader类的getSystemClassLoader()方法。
  ...
  JavaCalls::call_static(&result, // 调用Java静态方法的返回值，并将其存储在result中
                         ...
                         // 看Java代码：源代码位置：openjdk/jdk/src/share/classes/java/lang/ClassLoader.java
                         vmSymbols::getSystemClassLoader_name(),
                         // 调用目标方法的方法签名
                         vmSymbols::void_classloader_signature(),
                         CHECK);
  // 获取调用getSystemClassLoader()方法的返回值并将其保存到_java_system_loader
  // 属性中初始化属性为应用类加载器/AppClassLoader
...
// SystemDictionary::resolve_or_fail()函数在前面介绍过，该函数会遵循双亲委派机制加载类，通常会创建或从Dictionary中查询已经加载的InstanceKlass实例，不涉及对类的连接和初始化等操作。
Klass* SystemDictionary::resolve_or_fail(Symbol* class_name, Handle class_loader, Handle protection_domain, bool throw_error, TRAPS) {
  Klass* klass = resolve_or_null(class_name, class_loader, protection_domain, THREAD);
  // 如果之前已经产生了异常或klass为空，则抛出异常
  // 调用resolve_or_fail()函数加载类，但是klass一定不能为空，如果为空则抛出异常
...
// 调用resolve_or_null()函数时，即使klass为空也不会抛出异常
Klass* SystemDictionary::resolve_or_null(Symbol* class_name, Handle class_loader, Handle protection_domain, TRAPS) {
  ...
  if (FieldType::is_array(class_name)) {
    // 调用resolve_array_class_or_null()函数查找数组时，如果数组元素的类型为对象类型，则同样会调用resolve_instance_class_or_null()函数查找类对应的Klass实例
...
Klass* SystemDictionary::resolve_instance_class_or_null(Symbol* name,
  ...
  // 如果类还没有加载，那么当前的函数还需要负责加载类。在实现的过程中考虑的因素比较多，比如解决并行加载、触发父类的加载和域权限的验证等，不过这些都不是要讨论的重点
  ...
  if (!class_has_been_loaded) {
    ...
    if (!class_has_been_loaded) {
      ...
      // Do actual loading
      // 当类没有加载时，调用load_instance_class()函数进行加载
...
// find_instance_or_array_klass()函数包含对数组和类的查询逻辑，并不涉及类的加载
// 如果是基本类型，则调用Universe::typeArrayKlassObj()函数找到TypeArrayKlass实例，
...
// 调用SystemDictionary::resolve_from_stream()函数生成Klass实例
Klass* SystemDictionary::resolve_from_stream(Symbol* class_name,
  ...
  // 调用parseClassFile()函数完成类的解析之后会生成Klass实例，
  // 调用find_or_define_instance_class()或define_instance_class()函数可以将Klass实例注册到SystemDictionary中
...
// 当class_loader为NULL时，表示使用启动类加载器加载类，调用ClassLoader::load_classfile()函数加载类；当class_loader不为NULL时，
// 会调用java.lang.ClassLoader类中的loadClass()方法加载。这种判断逻辑也是“双亲委派”机制的体现。
// 在装载一个类时可能还会调用SystemDictionary::load_instance_class()函数，并且在该函数中可能会调用load_classfile()函数
instanceKlassHandle SystemDictionary::load_instance_class(Symbol* class_name, Handle class_loader, TRAPS) {
  ...
  if (class_loader.is_null()) {
    ...
    // 将生成的Klass实例保存起来Dictionary的底层是Hash表数据结构，使用开链法解决Hash冲突
    if (!k.is_null()) {
      // 支持并行加载，也就是允许同一个类加载器同时加载多个类
      // 调用的SystemDictionary::find_or_define_instance_class()函数会将这个Klass实例添加到字典中
...
// 将已经加载的类添加到系统词典SystemDictionary里
void SystemDictionary::define_instance_class(instanceKlassHandle k, TRAPS) {
 ...
 if (!class_loader_h.is_null() && !is_parallelCapable(class_loader_h)) {
  ...
  // Add the new class. We need recompile lock during update of CHA.
  {
    ...
    // Grabs and releases SystemDictionary_lock
    //define_instance_class()函数最终会调用SystemDictionary::update_dictionary()函数将已经加载的类添加到系统词典SystemDictionary里
    update_dictionary(d_index, d_hash, p_index, p_hash,
...
// 调用的SystemDictionary::find_or_define_instance_class()函数会将这个Klass实例添加到字典中
// 其中，find_or_define_instance_class()函数同样会调用find_class()函数从字典中检查这个类是否已经保存到系统字典中（因为并行加载的原因，其他线程可能已经创建了InstanceKlass实例并将其保存到字典中了）
// 如果找到实例就直接返回，否则调用define_instance_class()函数定义一个InstanceKlass实例并将其保存到字典中
// define_instance_class()函数最终会调用SystemDictionary::update_dictionary()函数将已经加载的类添加到系统词典SystemDictionary里
...
bool SystemDictionary::do_unloading(BoolObjectClosure* is_alive, bool clean_alive) {
  ...
  // 调用ClassLoaderDataGraph::do_unloading()函数找出失效的类加载器
...
bool SystemDictionary::initialize_wk_klass(WKID id, int init_opt, TRAPS) {
  ...
  if ((*klassp) == NULL) {
    // 调用resolve_or_fail()或resolve_or_null()函数进行类的加载时，最终会调用SystemDictionary::load_instance_class()函数
    // 例如在加载核心类时调用链路如下：
    /*
...
// 首先会调用System-Dictionary::initialize_wk_klasses_until()函数遍历WK_KLASSES_DO宏中表示的所有需要预加载的类
...
// SystemDictionary::initialize_preloaded_classes()函数分批次预加载类。
