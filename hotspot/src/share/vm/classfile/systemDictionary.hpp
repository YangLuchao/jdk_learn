...
// 在宏WK_KLASSES_DO中定义每个需要加载的核心类时也会指定InitOption的值，这些值会影响类加载的行为
...
// ClassLoaderData中都保存着一个私有的Dictionary，
// 而SystemDictionary只是一个拥有很多静态函数的工具类而已。
class SystemDictionary : AllStatic {
  friend class VMStructs;
  friend class SystemDictionaryHandles;

 public:
  // 通过宏定义了需要预加载的类，这个宏在枚举类WKID中使用
  /*
  将宏扩展后，变为如下形式
  enum WKID {
  NO_WKID = 0,
  ...
  ClassLoader_klass_knum, java_lang_ClassLoader_knum = ClassLoader_klass_
knum, \
  ...
  ...
  */
  ...
  enum InitOption {
    // 标记为Pre和Pre_JSR292的类会调用resolve_or_fail()函数进行预加载,如果类不存在，则报错
    ...
    // resolve_or_null()函数进行预加载，如果不存在，则返回NULL
    Opt,                        // preload tried; NULL if not present
    Opt_Only_JDK14NewRef,       // preload tried; use only with NewReflection
    Opt_Only_JDK15,             // preload tried; use only with JDK1.5+
    OPTION_LIMIT,
    // 在SystemDictionary::initialize_wk_klasses_until()函数或SystemDictionary::initialize_wk_klasses_until中用到的CEIL_LG_OPTION_LIMIT是枚举变量
...
protected:
  ...
  // HotSpot VM会将已经加载的类存储在Dictionary中
