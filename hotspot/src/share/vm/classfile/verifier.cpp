...
// 验证字节码
/*
JDK 6之后的Javac编译器给方法体的Code属性的属性表中增加了一项名为StackMapTable的属性，通过这项属性进行类型检查验证比类型推导验证的字节码验证过程更快。
...
在openjdk/hotspot/src/share/vm/classfile/verifier.cpp文件中定义了一系列verify_xxx()函数，它们都用来对字节码进行验证，有兴趣的读者可自行查阅。
*/
bool Verifier::verify(instanceKlassHandle klass, Verifier::Mode mode, bool should_verify_class, TRAPS) {
  ...
  // can_failover表示失败回退，对于小于NOFAILOVER_MAJOR_VERSION主版本号（值为51）的Class文件，
  ...
  if (is_eligible_for_verification(klass, should_verify_class)) {
    ...
    if (klass->major_version() >= STACKMAP_ATTRIBUTE_MAJOR_VERSION) {
      ...
      if (can_failover && !HAS_PENDING_EXCEPTION &&
        ...
        // 使用类型检查，如果失败，则使用类型推导验证can_failover为true时表示主版本号必定小于51，因此只有50版本允许回退到类型推导验证
