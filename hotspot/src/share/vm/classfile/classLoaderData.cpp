...
// 判断一个类是否需要被卸载
// 如果是匿名类加载器，并且类加载器加载的Klass实例对应的java.lang.Class标记为存活，那么该类加载器就不能卸载。
// 根类加载器或当前的java.lang.ClassLoader被标记，则这些类加载器也是存活状态。
...
// 调用类加载器的metaspace_non_null()函数获取Metaspace实例
...
// 将类装入器数据从主列表移动到卸载列表，以便稍后卸载和回收。
// 通过定义在ClassLoaderDataGraph类中的_unloading静态属性保存，多个失效的类加载器会形成一个单链表
...
void ClassLoaderDataGraph::purge() {
  ...
  while (next != NULL) {
    ...
    next = purge_me->next();
    // 使用delete关键字删除ClassLoaderData时，调用ClassLoaderData对应的析构函数
    delete purge_me;
  }
  // 元空间清洗
  Metaspace::purge();
