...
class ClassPathEntry: public CHeapObj<mtClass> {
 ...
 public:
  ...
  // 直到查找到某个类路径下名称为name的Class文件为止，
  // 这时open_stream()函数会返回定义此类的Class文件的ClassFileStream实例。
...
// 引导类加载器由ClassLoader类实现，这个ClassLoader类是用C++语言编写的，
// 负责将<JAVA_HOME>/lib目录、-Xbootclasspath选项指定的目录和系统属性sun.boot.class.path指定的目录下的核心类库加载到内存中
