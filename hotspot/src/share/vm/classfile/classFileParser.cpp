...
...
#5 = Utf8          TestClass
假设JVM_CONSTANT_Class是常量池第一项，则解析完这一项后的ConstantPool对象如test/ylctest/4/4.2所示。
*/
void ClassFileParser::parse_constant_pool_entries(int length, TRAPS) {
  ...
  // 循环处理length个常量池项，但第一个常量池项不需要处理，因此循环下标index的值初始化为1。
  for (int index = 1; index < length; index++) {
    // Each of the following case guarantees one more byte in the stream
    // for the following tag or the access_flags following constant pool,
    // so we don't need bounds-check for reading tag.
    // 常量池项的第一个字节都是用来描述常量池项类型的tag属性，调用cfs->get_u1_fast()函数获取常量池项类型后，就可以通过switch语句分情况进行处理。
    u1 tag = cfs->get_u1_fast();
    // 获取常量池项中的tag属性值
    switch (tag) {
      // 1. JVM_CONSTANT_Class项的解析
      /*
      CONSTANT_Class_info项的格式如下
      CONSTANT_Class_info {
        u1 tag;
        u2 name_index;
      }
      */
     // CONSTANT_Class_info项与JVM_CONSTANT_Class项的常量值相同，因此JVM_CONSTANT_Class常量池项的结构按照CONSTANT_Class_info的结构解析即可，后面的情况也类似
      case JVM_CONSTANT_Class :
        {
          cfs->guarantee_more(3, CHECK);  // name_index, tag/access_flags
          // 调用cfs->get_u2_fast()函数获取name_index
          u2 name_index = cfs->get_u2_fast();
          // 调用_cp->klass_index_at_put()函数保存name_index
          // _cp的类型为ConstantPool*
          // 在klass_index_at_put()函数中，tags数组中存储的是JVM_CONSTANT_ClassIndex，这个值在这一轮for循环处理完所有的常量池项后还会进一步处理一部分常量池项，
          // 主要是验证一些值的合法性并且更新JVM_CONSTANT_ClassIndex和JVM_CONSTANT_StringIndex常量池项中存储的值。
          _cp->klass_index_at_put(index, name_index);
        }
        break;
      // 2. CONSTANT_Fieldref_info项的解析
      /*
      CONSTANT_Fieldref_info项的格式如下
      ...
      case JVM_CONSTANT_Fieldref :
        {
          ...
          // 调用了field_at_put()函数存储class_index与name_and_type_index属性的值
          // 由于ConstantPool数据区的一个槽是一个指针类型的宽度，所以至少有32个位，又由于class_index与name_and_type_index属性的类型为u2，所以使用高16位存储name_and_type_index、低16位存储class_index即可。
          _cp->field_at_put(index, class_index, name_and_type_index);
        }
        break;
      // 3. JVM_CONSTANT_Methodref项的解析
      /*
      CONSTANT_Methodref_info项的格式如下：
      ...
      */
      case JVM_CONSTANT_Methodref :
        {
          cfs->guarantee_more(5, CHECK);  // class_index, name_and_type_index, tag/access_flags
          // 按照格式读取Class文件，获取相关属性值后调用ConstantPool类的method_at_put()函数进行存储
          u2 class_index = cfs->get_u2_fast();
          u2 name_and_type_index = cfs->get_u2_fast();
          // method_at_put()函数的实现逻辑与field_at_put()函数的实现逻辑类似，都是使用一个槽的高16位存储name_and_type_index，低16位存储class_index
          _cp->method_at_put(index, class_index, name_and_type_index);
        }
        break;
      // 4. JVM_CONSTANT_InterfaceMethodref项的解析
      /*
      CONSTANT_InterfaceMethodref_info项的格式如下：
      ...
      */
      case JVM_CONSTANT_InterfaceMethodref :
        ...
        break;
      // 5. JVM_CONSTANT_String项的解析
      /*
      CONSTANT_String_info项的格式如下
      ...
      case JVM_CONSTANT_String :
        {
          ...
          // 之后也会对JVM_CONSTANT_StringIndex进行修正
          _cp->string_index_at_put(index, string_index);
        }
        break;
      // 6. JVM_CONSTANT_MethodHandle项的解析
      /*
      CONSTANT_MethodHandle_info项的格式如下：
      ...
      */
      case JVM_CONSTANT_MethodType :
        ...
        break;
      // 8. JVM_CONSTANT_InvokeDynamic项的解析
      /*
      CONSTANT_InvokeDynamic_info项的格式如下
      ...
      */
      case JVM_CONSTANT_InvokeDynamic :
        ...
        break;
      //  9.JVM_CONSTANT_Integer项与JVM_CONSTANT_Float项的解析
      /*
      CONSTANT_Integer_info项与CONSTANT_Float_info项的格式如下
      ...
      */
      ...
      case JVM_CONSTANT_Float :
        ...
        break;
      // 10. JVM_CONSTANT_Long项与JVM_CONSTANT_Double项的解析
      /*
      ...
      */
      ...
      case JVM_CONSTANT_Double :
        ...
        index++;   // Skip entry following eigth-byte constant, see JVM book p. 98
        break;
      // 11. JVM_CONSTANT_NameAndType项的解析
      /*
      CONSTANT_NameAndType_info项的格式如下
      ...
      */
      case JVM_CONSTANT_NameAndType :
        ...
        break;
      // 12. JVM_CONSTANT_Utf8项的解析
      /*
      CONSTANT_Utf8_info项的格式如下
      CONSTANT_Utf8_info {
        u1 tag;
        u2 length;
        u1 bytes[length];
      ...
      */
     // 在ConstantPool数据区相应槽位上只需要存储指向Symbol的指针
      case JVM_CONSTANT_Utf8 :
        {
          ...
          unsigned int hash;
          // 调用SymbolTable::lookup_only()函数从字典表中查找对应的Symbol实例
          Symbol* result = SymbolTable::lookup_only((char*)utf8_buffer, utf8_length, hash);
          if (result == NULL) {
            // 如果查不到需要暂时将相关的信息存储到临时的names、lengths、indices与hashValues数组中
            ...
            // 如果找到对应的Symbol实例，则调用symbol_at_put()函数向ConstantPool数据区对应槽位上存储指向Symbol实例的指针
...
constantPoolHandle ClassFileParser::parse_constant_pool(TRAPS) {
  ...
  // 调用ConstantPool::allocate()函数创建ConstantPool实例
  ...
  // 解析常量池项
  // 调用parse_constant_pool_entries()函数解析常量池项并将这些项保存到ConstantPool实例中
  ...
  for (index = 1; index < length; index++) {          // Index 0 is unused
    ...
    switch (tag) {
      ...
      // JVM_CONSTANT_ClassIndex，更新逻辑
        {
          // 调用klass_index_at()函数获取ConstantPool数据区index槽位上存储的值，对于实例4-2来说，这个值为5。
          ...
          // symbol_at()函数将_tags数组中保存的tag值更新为JVM_CONSTANT_UnresolvedClass
          // 更新后的ConstantPool实例布局如图test/ylctest/4/4.2/ConstantPool实例布局3.png
          cp->unresolved_klass_at_put(index, cp->symbol_at(class_index));
        }
        break;
      case JVM_CONSTANT_StringIndex :
      // JVM_CONSTANT_StringIndex更新逻辑
        {
          // 更新了ConstantPool数据区对应槽位上的值，这个值之前是一个对CONSTANT_Utf8_info常量池的索引，
          // 所以和CONSTANT_Class_info项的处理一样，将对应槽位上的值更新为直接指向Symbol实例的指针。
...
Array<Klass*>* ClassFileParser::parse_interfaces(int length,
  ...
  if (length == 0) {
    ...
    for (index = 0; index < length; index++) {
      ...
      // 如何通过interface_index找到对应的InstanceKlass实例
      if (_cp->tag_at(interface_index).is_klass()) {
        ...
        // 通过interface_index找到接口在C++类中代表的实例InstanceKlass，
        ...
        Klass* k = SystemDictionary::resolve_super_or_fail(class_name,
                      ...
                      false, CHECK_NULL);
        // 然后封装为KlassHandle并存储到_local_interfaces数组中                      
...
// 用来统计静态与非静态情况下5种类型变量的数量，这样在分配内存空间时会根据变量的数量计算所需要的内存空间
/*
Oop：引用类型；
Byte：字节类型；
Short：短整型；
...
// BasicType主要是针对Java类型进行定义的，而HotSpot VM中只有FieldAllocationType枚举类中定义的5种类型，因此要对Java类型进行转换后再统计
// T_CONFLICT枚举常量的值为19
...
class FieldAllocationCount: public ResourceObj {
 public:
  // count数组用来统计静态与非静态情况下各个类型变量的数量，这些类型通过Field-AllocationType枚举类定义
  u2 count[MAX_FIELD_ALLOCATION_TYPE];

  // MAX_FIELD_ALLOCATION_TYPE是定义在FieldAllocationType中的枚举常量，值为10初始化count数组中的值为0
  ...
  // 更新对应类型字段的总数量
  // FieldAllocationCount类中定义的update()函数用来更新对应类型变量的总数量。其中，BasicType枚举类
...
Array<u2>* ClassFileParser::parse_fields(Symbol* class_name,
  ...
  *java_fields_count_ptr = length;
  ...
  int num_injected = 0;
  // 注入字段
  // 注入字段基础知识看test/ylctest/5/5.4/字段注入
  // 调用JavaClasses::get_injected()函数得到注入字段的数量并保存到num_injected中，将num_injected也记入总的字段数量total_fields中。
  /*
  函数JavaClasses::get_injected()最终返回的代码
{ SystemDictionary::java_lang_Class_knum,vmSymbols::klass_name_enum,
  ...
  */
  ...
  ResourceMark rm(THREAD);
  /*
  NEW_RESOURCE_ARRAY_IN_THREAD为宏，扩展后，以上的调用语句如下
  u2* fa = (u2*) resource_allocate_bytes(THREAD, (total_fields * (FieldInfo::field_slots + 1)) * sizeof(u2))
  FieldInfo::field_slots枚举常量的值为6，在内存中开辟total_fields * 7个sizeof(u2)大小的内存空间，因为字段f1、f2、...、fn在存储时要按如下的格式存储：
f1: [access, name index, sig index, initial value index, low_offset,high_offset]
f2: [access, name index, sig index, initial value index, low_offset,high_offset]
      ...
fn: [access, name index, sig index, initial value index, low_offset,high_offset]
    ...
    [generic signature index]
    ...
    也就是说，如果有n个字段，那么每个字段要占用6个u2类型的存储空间，不过每个字段还可能会有generic signature index（占用1个u2类型的存储空间），
...
field_info {
   ...
   其中，access_flags、name_index与descriptor_index对应的就是每个fn中的access、name index与sig index。
   另外，initial value index用来存储常量值（如果这个变量是一个常量），low_offset与high_offset可以保存该字段在内存中的偏移量
}
  */
  ...
  for (int n = 0; n < length; n++) {
    ...
    // 变量属性读取完毕
    // 按Java虚拟机规范规定的字段格式读取字段信息后将其存储到fa中，之前已经为fa变量分配好了存储空间，只需要按照上一节介绍的字段存储约定存储即可
    FieldInfo* field = FieldInfo::from_field_array(fa, n);
    // 调用field->initialize()函数存储读取的字段的各个值
    field->initialize(access_flags.as_short(),
                      name_index,
                      signature_index,
                      constantvalue_index);
    // 调用_cp->basic_type_for_signature_at()函数从变量的签名中读取类型
    BasicType type = _cp->basic_type_for_signature_at(signature_index);

    // Remember how many oops we encountered and compute allocation type
    // 对字段的数量进行统计
    // 得到字段的类型后，调用fac->update()函数增加对应类型的字段数量。
  ...
  // 在ClassFileParser::parse_fields()函数中通过for循环处理完所有字段后，接着将临时存储变量信息fa中的信息复制到新的数组中，以避免内存浪费
  // 在创建fields数组时可以看到，元素类型为u2的数组的大小变为index * FieldInfo::field_slots + num_generic_signature，其中的index表示字段总数。
  // 另外，根据实际情况分配了num_generic_signature的存储位置，然后将fa中存储的信息复制到fields中
...
Code_attribute {
  ...
  */
  ...
  // 循环读取多个属性
  // 代码相对简单，只需要按照规定的格式从Class文件中读取信息即可，这些读取出来的信息最终会存储到Method或ConstMethod实例中，供HotSpot VM运行时使用。
  ...
  // ConstMethod内存布局如图：test/ylctest/6/6.2/ConstMethod实例的内存布局.png
  // 方法的字节码存储在紧挨着ConstMethod本身占用的内存空间之后，在方法解释运行时会频繁从此处读取字节码信息。
  ...
  // 将前面从Class文件中解析出的相关属性信息设置到Method实例中进行保存
  m->set_constants(_cp);
...
Array<Method*>* ClassFileParser::parse_methods(bool is_interface,
  ...
  if (length == 0) {
    ...
    for (int index = 0; index < length; index++) {
      // 调用parse_method()函数解析每个Java方法，该函数会返回表示方法的Method实例，但Method实例需要通过methodHandle句柄来操作，
      // 因此最终会封装为methodHandle句柄，然后存储到_methods数组中
...
// layout_fields()函数进行字段布局的实现过程。传入的fac是之前介绍的FieldAllocationCount类型的变量，里面已经保存了各个类型字段的数量。
// 计算静态字段的偏移量、非静态字段的偏移量、各个字段在oop实例中的偏移量
void ClassFileParser::layout_fields(Handle class_loader,
  ...
  // 调用InstanceMirrorKlass::offset_of_static_fields()函数获取_offset_of_static_fields属性的值
  ...
  if ( fac->count[STATIC_DOUBLE] &&
    ...
    // 计算next_static_double_offset时，因为首先布局的是oop，内存很可能不是按8字节对齐，所以需要调用align_size_up()函数对内存进行8字节对齐
    next_static_double_offset = align_size_up(next_static_double_offset, BytesPerLong);
  }
  // 按照oop、double、word、short和byte的顺序计算各个静态字段的偏移量，
  // next_static_xxx_offset指向的就是第一个xxx类型的静态变量在oop实例（表示java.lang.Class对象）中的偏移量
  // 可以看到，在fac中统计各个类型字段的数量就是为了方便在这里计算偏移量
  ...
  // 非静态字段起始偏移量计算
  // 因为非静态字段存储在instanceOopDesc实例中，并且父类字段存储在前，
  // 所以nonstatic_fields_start变量表示的就是当前类定义的实例字段所要存储的起始偏移量位置
  // 子类会复制父类中定义的所有非静态字段（包括private修饰的非静态字段），以实现字段继承。
  // 因此上面在计算子类非静态字段的起始偏移量时会将父类可被继承的字段占用的内存也考虑在内
  // oop字段内存布局如test/ylctest/5/5.3/oop字段内存布局.png所示
  ...
  // 在计算非静态字段的偏移量时还需要考虑有@Contended注解的类和字段。对于类上的@Contended注解，
  // 需要在字段之前填充ContendedPaddingWidth字节，对于有@Contended注解的变量来说，还需要单独考虑布局，因此相关实例字段的数量需要分别进行统计
  ...
  // 这里涉及对有@Contended注解的实例字段的处理，为了避免伪共享的问题，可能需要在两个字段的存储布局之间做一些填充。
  ...
  // 这几个变量与OopMapBlock的实现逻辑有关---------------------
  // 其中，nonstatic_oop_offsets和nonstatic_oop_counts可存储多个OopMapBlock信息。
  int* nonstatic_oop_offsets;
  unsigned int* nonstatic_oop_counts;
  //  保存OopMapBlock的数量
...
另外，由于填充会形成空隙，比如使用压缩指针时oop对象头占12字节，后面如果是long类型字段的话，
long的对齐要求是8字节，中间会有4个字节的空隙，为了提高内存利用率，可以把int、short、byte和oop等相对内存占用比较小的对象填充进去，
...
*/
  ...
  if( allocation_style == 0 ) {
    ...
    // 并且父类将oop布局在末尾位置时可使用allocation_style=0来布局，这样子类会首先将自己的oop布局在开始位置，正好和父类的oop连在一起，
    // 有利于GC扫描处理引用。剩下的其他情况都是按allocation_style属性值为1来布局的，也就是oop在末尾。
    // 如果父对象有oop字段并且oop字段布局到了末尾，那么尽量应该将本对象的实例字段布局到开始位置，让父对象的oop与子对象的oop挨在一起
    // Fields allocation: oops fields in super and sub classes are together.
    if( 
      // nonstatic_field_size指的是父类的非静态变量占用的大小
      nonstatic_field_size > 0 && _super_klass() != NULL &&
  ...
  // long/double类型占用8字节，对齐时最多可能留下7字节的空白
  ...
  if( nonstatic_double_count > 0 ) {
    ...
    if( compact_fields && offset != next_nonstatic_double_offset ) {
      ...
      // nonstatic_word_count记录了word的总数，由于这个空隙算一个特殊位置，因此把放入这里的word从正常情况删除，并加入特殊的nonstatic_word_space_count中
      ...
      // heapOopSize在开启指针压缩时为4，否则为8，所以一个oop占用的字节数要看heapOopSize的大小，
      // 理论上空隙最多只能存放一个oop实例allocation_style必须不等于0，
      // 因为等于0时oop要分配到开始的位置和父类的oop进行连续存储，不能进行空隙填充
  ...
  // 将各个类型字段在instanceOop实例中的偏移量计算出来后，下面就可以计算每个字段的实际偏移量位置了
  // 计算各个字段在oop实例中的偏移量
  for (AllFieldStream fs(_fields, _cp); !fs.done(); fs.next()) {
    ...
    switch (atype) {
      ...
      case NONSTATIC_OOP:
        ...
        // Update oop maps
        // 生成OopMapBlock需要的信息
        // 通过first_nonstatic_oop_offset变量保存第一个对象类型字段的实际偏移量，
        // 通过nonstatic_oop_counts[nonstatic_oop_map_count - 1]保存对象类型字段的总数。
    ...
    // 由于实例字段会填充到空隙中，所以还需要考虑这一部分的字段，对计算出来的偏移量连续存储即可。
    // 最终计算出来的每个字段的偏移量要调用fs.set_offset()保存起来，这样就能快速找到这些字段的存储位置了。
  }
  ...
  if (nonstatic_contended_count > 0) {
    ...
    // 同为一组的有@Contended注解的字段要布局在一起。
    // 同一组的字段可能类型不同，并且也不会遵循前面介绍的对实例字段的布局策略，因此需要在每次开始之前调用align_size_up()函数进行对齐操作。
    // 布局完一组后要填充ContendedPaddingWidth个字节，然后使用相同的逻辑布局下一组字段。
    // 最终的字段偏移量同样会调用fs.set_offset()函数保存起来，以方便后续根据字段偏移量进行查找。
    int current_group = -1;
  ...
  const unsigned int total_oop_map_count =
    // 调用compute_oop_map_count()函数计算当前InstanceKlass实例需要的OopMapBlock数量
...
// 加载并解析Class文件
// ClassFileParser::parseClassFile()函数的实现代码比较多，但却是严格按照Java虚拟机规范定义的Class文件格式解析的。
instanceKlassHandle ClassFileParser::parseClassFile(Symbol* name,
  ...
  // 1. 解析魔数、次版本号与主版本号
  // Magic value
  // 首先读取魔数并验证值是否为0xCAFEBABE，然后读取Class文件的次版本号和主版本号并保存到ClassFileParser实例的_minor_version和_major_version属性中
  // 解析魔数
  // 1.验证magic的值必须为0xCAFEBABY
  ...
  // 从文件流中读取相关内容时，通常会调用guarantee_more()函数以保证文件流中有足够的字节内容。
  cfs->guarantee_more(8, CHECK_(nullHandle));  // flags, this_class, super_class, infs_len
  ...
  AccessFlags access_flags;
  // 获取访问标识
  // 读取并验证了访问标识，与JVM_RECOGNIZED_CLASS_MODIFIERS进行与运算操作的目的是过滤掉一些非法的访问标识。
  // JVM_RECOGNIZED_CLASS_MODIFIERS是一个宏，看定义
  ...
  // 将读取的当前类的名称（通过Symbol实例表示）保存到ClassFileParser实例的_class_name属性中
  ...
  { HandleMark hm(THREAD);
...
#endif
    // 4.解析父类索引
    // 父类索引（super_class）是一个u2类型的数据，用于确定当前正在解析的类的父类全限定名。由于Java语言不允许多重继承，所以父类索引只有一个
    // 父类索引指向常量池中类型为CONSTANT_Class_info的类描述符，再通过类描述符中的索引值找到常量池中类型为CONSTANT_Utf8_info的字符串
    ...
    // Interfaces
    // 5. 解析实现接口
    // 接口表interfaces[interfaces_count]中的每个成员的值必须是一个对constant_pool表中项目的有效索引值，该索引值在常量池中对应CONSTANT_Class_info常量池项
    // 接口表中的每个成员所表示的接口顺序和对应的Java源代码中给定的接口顺序（从左至右）一致，即interfaces[0]对应的是Java源代码中最左边的接口。
    // 接口数量
    ...
    FieldAllocationCount fac;
    // 调用parse_classfile_attributes()函数解析类字段，过程比较烦琐，只需要按照各属性的格式解析即可
    ...
    if (super_klass.not_null()) {
      ...
      // 在openjdk/hotspot/src/share/vm/classfile/classFileParser.cpp文件中
      // 定义了一系列verify_xxx()和check_xxx()函数，它们都用来对元数据进行验证
      // 保证父类不为接口
    ...
    klassVtable::compute_vtable_size_and_num_mirandas(
        // 调用ClassFileParser::parseClassFile()函数时传递的methods就是调用parse_methods()函数后的返回值，
        // 数组中存储了类或接口中定义或声明的所有方法，不包括父类和实现的接口中的任何方法
        // miranda方法就是编译器在子类中实现接口的def方法但没有重写的方法
    ...
    // 计算itable的大小
    itable_size = 
    // 当为接口时，itable_size的值为0
    // compute_itable_size会调用visit_all_interfaces()函数，这是第一调用计算接口和接口中需要存储在itable中的方法总数
    // 后面再初始化itable(klassItable::setup_itable_offset_table)的时候会第二次调用
    access_flags.is_interface() ? 0 : klassItable::compute_itable_size(_transitive_interfaces);
    ...
    // ClassFileParser::parseClassFile()函数会将解析Class文件的大部分结果保存到Instance-Klass实例
    ...
    int total_oop_map_size2 =
          // 调用nonstatic_oop_map_size()函数计算多个OopMapBlock在InstanceKlass实例中需要占用的内存空间
          // 最终oop_map_count个OopMapBlock会存储到InstanceKlass实例中的itable之后，
    ...
    // 调用InstanceKlass::allocate_instance_klass()函数的实现代码在2.1.3节中介绍过，
    // 首先是计算InstanceKlass实例需要占用的内存空间，然后在堆中分配内存，最后调用构造函数创建实例
    _klass = InstanceKlass::allocate_instance_klass(loader_data,
                                                    vtable_size,
                                                    itable_size,
                                                    info.static_field_size,
                                                    // 计算出OopMapBlock需要占用的内存空间后调用InstanceKlass::allocate_instance_klass()函数分配内存，
                                                    // 该函数在创建InstanceKlass实例时为多个OopMapBlock预留的内存空间就是total_oop_map_size2的内存空间
                                                    total_oop_map_size2,
                                                    rt,
                                                    access_flags,
                                                    name,
                                                    ...
                                                    !host_klass.is_null(),
                                                    CHECK_(nullHandle));
    // 将InstanceKlass封装为句柄进行操作instanceKlassHandle                                                    
    ...
    // 通过set_nonstatic_field_size保存非静态字段需要占用的内存空间，在计算oop实例（表示Java对象）需要占用的内存空间时非常重要，因为非静态字段存储在oop实例中
    ...
    // Klass::initialize_supers()函数会初始化_primary_supers、_super_check_offset、_secondary_supers与_secondary_super_cache属性，以加快判断类之间的关系
    ...
    // 调用fill_oop_maps()函数填充OopMapBlock的信息
    ...
    // InstanceKlass实例保存了Class文件解析的结果，每个InstanceKlass实例还会有java.lang.Class对象，
    // 通过oop实例来表示，因此在设置完InstanceKlass实例的相关属性后，会调用create_mirror()函数创建oop实例。
...
// 调用fill_oop_maps()函数填充OopMapBlock的信息
// 复制父类的OopMapBlock信息，如果能扩展父类的最后一个OopMapBlock代表父类和子类连续的对象类型变量区域，
// 则不需要为当前类中定义的对象类型变量新创建一个OopMapBlock，否则就新创建一个。
