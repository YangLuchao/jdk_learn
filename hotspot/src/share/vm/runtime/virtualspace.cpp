...
void ReservedSpace::initialize(size_t size, size_t alignment, bool large,
  ...
  if (base == NULL) {
    ...
    if (requested_address != 0) {
      ...
      // 我们只讨论不使用指针压缩的情况，因此requested_address变量的值为0，即调用os::reserve_memory()函数分配内存，
      // os::reserve_memory()函数最终会调用anon_mmap()函数分配内存
      // 如果分配成功，最终会返回内存大小为bytes的基地址addr，这样就可以在GenCollectedHeap::initialize()函数中创建MemRegion对象，为年轻代和老年代分配内存空间了
...
// ReservedHeapSpace的构造函数
ReservedHeapSpace::ReservedHeapSpace(size_t size, size_t alignment,
                                     bool large, char* requested_address) :
  ReservedSpace(size,   // 需要分配的内存大小
  alignment,            // 内存需要按alignment进行对齐
  large,                // 默认值为false，表示不使用大页，传递的值为NULL，表示对分配的堆基址没有任何要求
                requested_address,
