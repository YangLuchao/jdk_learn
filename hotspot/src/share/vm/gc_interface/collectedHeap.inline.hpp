...
void CollectedHeap::post_allocation_setup_no_klass_install(KlassHandle klass,
  ...
  // 其锁状态一般为无锁状态Klass中的_prototype_header完全是为了支持偏向锁增加的属性，后面章节中会详细介绍偏向锁的实现机制
  if (UseBiasedLocking && (klass() != NULL)) {
    obj->set_mark(klass->prototype_header());
  } else {
    // May be bootstrapping
    // markOopDesc::prototype()函数初始化的对象头
...
// 调用CollectedHeap::obj_allocate()函数可以创建real_klass的oop实例(java.lang.Class对象)
...
// 在堆上分配内存空间，然后初始化对象头信息
// CollectedHeap::array_allocate()和CollectedHeap::array_allocate_nozero()两个函数的实现类似，我们只看CollectedHeap::array_allocate()函数的实现即可
oop CollectedHeap::array_allocate(KlassHandle klass,
  ...
  assert(!Universe::heap()->is_gc_active(), "Allocation during gc not allowed");
  assert(size >= 0, "int won't convert to size_t");
  // 调用common_mem_allocate_init()函数在堆上分配指定size大小的内存
  HeapWord* obj = common_mem_allocate_init(klass, size, CHECK_NULL);
  // 调用post_allocation_setup_array()函数初始化对象头
