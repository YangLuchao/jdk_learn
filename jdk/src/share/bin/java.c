...
// ylc java的main方法
// 1、调用InitializeJVM()函数初始化JVM，主要是初始化两个非常重要的变量JavaVM与JNIEnv。这里不过多介绍，后面在讲解JNI调用时会详细介绍初始化过程。
...
// 3、调用GetStaticMethodId()函数查找Java启动方法，其实就是获取Java主类中的main()方法。
...
// 5、以上步骤都在当前线程的控制下。当控制权转移到Java主类中的main()方法之后，当前线程就不再做其他事情了，等main()方法返回之后，当前线程会清理和关闭JVM，调用本地函数jni_DetachCurrentThread()断开与主线程的连接。当成功与主线程断开连接后，当前线程会一直等待程序中的所有非守护线程全部执行结束，然后调用本地函数jni_DestroyJavaVM()对JVM执行销毁
int JNICALL
JavaMain(void * _args)
{
    ...
    // LoadJavaVM()函数中指向libjvm.so动态链接库中的JNI_CreateJavaVM()函数
 ...
 */
// 执行Java主类的main()方法之前需要先在JavaMain()函数（定义在openjdk/jdk/src/share/bin/java.c文件中）中调用LoadMainClass()函数装载Java主类
static jclass
LoadMainClass(JNIEnv *env, int mode, char *name)
{
    ...
    // 调用GetStaticMethodID()函数可以查找Java启动方法（Java主类中的main()方法）的methodID。
    NULL_CHECK0(mid = (*env)->GetStaticMethodID(env, cls,
                "checkAndLoadMain",
                // 参数是：boolean , int, String 返回值是Class
                "(ZILjava/lang/String;)Ljava/lang/Class;"));
    str = NewPlatformString(env, name);
    CHECK_JNI_RETURN_0(
        // 调用sun.launcher.LauncherHelper类中的checkAndLoadMain()方法
        // CallStaticObjectMethod()函数会通过jni_invoke_static()函数执行checkAndLoadMain()方法
