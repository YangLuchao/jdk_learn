 ...
 */
// HotSpot VM在启动的过程中会在<JAVA_HOME>/lib/rt.jar包里的sun.misc.Launcher类中完成扩展类加载器和应用类加载器的实例化，
// 并会调用C++语言编写的ClassLoader类的initialize()函数完成应用类加载器的初始化。
public class Launcher {
    private static URLStreamHandlerFactory factory = new Factory();
    // 静态发射器
    // 在定义静态变量时就会初始化Launcher对象。调用的Launcher构造函数
    ...
    // 在Launcher类的构造方法中创建ExtClassLoader与AppClassLoader对象，
    // 而loader变量被初始化为AppClassLoader对象，
    // 最终在initSystemClassLoader()函数中调用getClassLoader()方法返回的就是这个对象
    public Launcher() {
     ...
     */
    // 在Launcher类的构造方法中实例化应用类加载器AppClassLoader时，
    // 会调用getAppClassLoader()方法获取应用类加载器，
    // 传入的参数是一个扩展类加载器ExtClassLoader对象，
    // 这样应用类加载器的父加载器就变成了扩展类加载器（与父加载器并非继承关系）。
