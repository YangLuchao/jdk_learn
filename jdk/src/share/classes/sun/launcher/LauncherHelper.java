...
public enum LauncherHelper {
    ...
    // 调用scloader的loadClass()方法会调用java.lang.ClassLoader的loadClass()方法，
    // 前面已经介绍过该方法，首先通过findLoadedClass()方法判断当前加载器是否已经加载了指定的类，
    // 如果没有加载并且parent不为NULL，则调用parent.loadClass()方法完成加载。
    // 而AppClassLoader的父加载器是ExtClassLoader，这是加载JDK中的扩展类，并不会加载Java主类，
    // 最终根类加载器也不会加载Java的主类（因为这个主类不在根类加载器负责加载的范围之内），
    // 因此只能调用findClass()方法完成主类的加载。对于AppClassLoader来说，调用的是URLClassLoader类中实现的findClass()方法，
     ...
     */
    // 如果我们为虚拟机指定运行主类为com.classloading/Test，
    // 那么这个参数会被传递到checkAndLoadMain()方法中作为what参数的值。
    // 这个类最终会通过应用类加载器进行加载，也就是AppClassLoader。
    public static Class<?> checkAndLoadMain(boolean printToStderr,
        ...
        try {
            // 根据类名称加载主类
            // scloader就是AppClassLoader，看scloader的定义
        ...
        // 在checkAndLoadMain()方法中调用AppClassLoader类加载器的loadClass()方法就完成了主类的加载，
        // 后续HotSpot VM会在主类中查找main()方法然后运行main()方法
