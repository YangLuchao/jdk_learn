...
FindBootStrapClass(JNIEnv *env, const char* classname)
{
   ...
   // 通过函数指针findBootClass来调用JVM_FindClassFromBootLoader()函数
   // 调用JVM_FindClassFromBootLoader()函数可以查找启动类加载器加载的类
   // 在JVM_FindClassFromBootLoader()函数中调用SystemDictionary::resolve_or_null()函数
   // 查找类sun.launcher.LauncherHelper，如果查找不到还会加载类
